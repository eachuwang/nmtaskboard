  ...
  app.post("/api/agent/sessions/:id/messages", asyncH(async (req, res) => {
    ...
    try {
      ...
      if (plan.tool === "draftTasks") {
        ...
        await sessions.save(req.context, session);
        const reply = `已生成 ${draft.tasks.length} 条任务草稿，请检查任务字段与标签后确认创建。`;
        send("draft", { draft });
      ...
      if (plan.tool === "draftTaskActions") {
        ...
        await sessions.save(req.context, session);
        const reply = `已生成 ${draft.actions.length} 项原子操作草稿，请检查状态、原因和进展内容后确认。`;
      ...
      if (plan.tool === "draftAssignments") {
        ...
        try {
          ...
          await sessions.save(req.context, session);
          const reply = `已生成「${draft.parent.title}」的分派草稿，请检查成员、截止日期和执行卡影响后确认。`;
