...
import { createRunEmitter, isTruncatedCompletion, RUN_PHASES, RUN_REASONS, stampDraftOrigin } from "../agent-protocol.js";
...
async function llmAvailability(ctx, context) {
  try {
    resolveActiveLlm(normalizeSettings(await ctx.persistence.settings.load(context)));
...
async function assertWritePlanAllowed(ctx, context, tool, args = {}) {
  try {
    await authorizeAgentTool(ctx, context, tool, args);
  } catch (error) {
    if (error.code === "AGENT_WRITE_TOOLS_DISABLED") {
      await auditTeamTool(ctx, context, tool, "denied", { code: error.code });
    }
    throw error;
  }
}
  ...
  app.post("/api/agent/sessions/:id/messages", asyncH(async (req, res) => {
    ...
    try {
      ...
      if (WRITE_PLAN_TOOLS.has(plan.tool)) {
        ...
        await assertWritePlanAllowed(ctx, req.context, plan.tool, plan.arguments);
