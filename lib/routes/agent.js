...
export function register(app, ctx) {
  ...
  app.post("/api/agent/sessions", asyncH(async (req, res) => {
    ...
    const session = {
      ...
      status: "active", createdAt: new Date().toISOString(), messages: [], drafts: [], actionDrafts: [], assignmentDrafts: []
    };
  ...
  app.post("/api/agent/sessions/:id/actions/:draftId/confirm", asyncH(async (req, res) => {
    ...
    if (!draft.confirmationPromise) {
      draft.status = "confirming";
      draft.confirmationPromise = confirmAgentActionDraft(ctx, req.context, draft)
        .then((result) => { draft.status = "confirmed"; draft.result = result; return result; })
        .catch((error) => { draft.status = "pending"; draft.confirmationPromise = null; throw error; });
    }
    const result = await draft.confirmationPromise;
    res.status(201).json({ result, replayed: false });
  }));
  ...
  app.post("/api/agent/sessions/:id/assignments/:draftId/confirm", asyncH(async (req, res) => {
    ...
    if (!draft.confirmationPromise) {
      draft.status = "confirming";
      draft.confirmationPromise = confirmAgentAssignmentDraft(ctx, req.context, draft)
        .then((result) => { draft.status = "confirmed"; draft.result = result; return result; })
        .catch(async (error) => {
          draft.status = "pending";
          draft.confirmationPromise = null;
  ...
  app.post("/api/agent/sessions/:id/messages", asyncH(async (req, res) => {
    ...
    try {
      ...
      if (plan.tool === "draftTaskActions") {
        ...
        const draft = createAgentActionDraft(extractJson(generated.content), loaded, req.context, plan.intent, text);
        session.actionDrafts.push(draft);
        session.actionDrafts = session.actionDrafts.slice(-6);
        send("actionDraft", { draft });
        send("tool", { name: plan.tool, status: "complete" });
        send("delta", { text: `已生成 ${draft.actions.length} 项原子操作草稿，请检查状态、原因和进展内容后确认。` });
        send("done", { model: llm.model });
        res.end();
        return;
      }
      if (plan.tool === "draftAssignments") {
        ...
        try {
          ...
          const draft = createAgentAssignmentDraft(extractJson(generated.content), loaded, listed.members, req.context, plan.intent);
          session.assignmentDrafts.push(draft);
          session.assignmentDrafts = session.assignmentDrafts.slice(-6);
          send("assignmentDraft", { draft });
          send("tool", { name: plan.tool, status: "complete" });
          send("delta", { text: `已生成「${draft.parent.title}」的分派草稿，请检查成员、截止日期和执行卡影响后确认。` });
          send("done", { model: llm.model });
        } catch (error) {
