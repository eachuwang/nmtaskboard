...
import { listAvailableModels, loadEffectiveLlmSettings, resolveActiveLlm, resolveLlmById } from "../settings.js";
...
async function activeAgentConfig(ctx, context, modelRef = "") {
  ...
  if (modelRef && typeof modelRef === "string") {
    ...
    const providerId = sep > 0 ? modelRef.slice(0, sep) : "";
    const modelId = sep > 0 ? modelRef.slice(sep + 1) : "";
    const known = listAvailableModels(settings).some((item) => item.id === modelRef);
    if (!providerId || !modelId || !known) throw agentError("AGENT_MODEL_INVALID", "所选模型不在已配置清单中", 400);
    return { settings, llm: resolveLlmById(settings, providerId, modelId) };
  }
...
export function register(app, ctx) {
  ...
  // NM Helper 可选模型清单（来自超管 LLM 配置；仅 id 与名称，无密钥）
  app.get("/api/agent/models", asyncH(async (req, res) => {
    const settings = await loadEffectiveLlmSettings(ctx.persistence, req.context);
    res.json({ models: listAvailableModels(settings) });
  }));
  ...
  app.post("/api/agent/sessions/:id/messages", asyncH(async (req, res) => {
    ...
    const { settings, llm } = await activeAgentConfig(ctx, req.context, req.body?.model);
      ...
      if (WRITE_PLAN_TOOLS.has(plan.tool)) {
        ...
        if (plan.tool === "draftTasks") {
          ...
          const generated = await chatCompletion({
            baseUrl: llm.baseUrl, apiKey: llm.apiKey, model: llm.model,
            temperature: llm.temperature,
            messages: agentTaskDraftPrompt(text, new Date().toISOString().slice(0, 10), settings.tags, req.context.statusWorkflow),
            jsonMode: true, timeoutMs: 90000, signal: ctrl.signal
          });
          if (run.closed) return;
          const draft = createAgentDraft(parseModelJson(generated), settings.tags, plan.intent, req.context.statusWorkflow);
        ...
        if (plan.tool === "draftTaskActions") {
          ...
          const generated = await chatCompletion({
            baseUrl: llm.baseUrl, apiKey: llm.apiKey, model: llm.model,
            temperature: llm.temperature,
            messages: agentTaskActionPrompt(text, visible, req.context.statusWorkflow),
            jsonMode: true, timeoutMs: 90000, signal: ctrl.signal
          });
          if (run.closed) return;
