...
export function register(app, ctx) {
  ...
  app.post("/api/report/versions", asyncH(async (req, res) => {
    ...
    if (!body.evidenceSummary || typeof body.evidenceSummary !== "object") {
      throw Object.assign(new Error("证据包摘要不能为空"), { statusCode: 400 });
    }
  ...
  app.get("/api/report/versions/:id", asyncH(async (req, res) => {
    ...
    if (!version) throw Object.assign(new Error("报告版本不存在或无权访问"), { statusCode: 404 });
    res.json({ version });
  }));
  ...
  app.get("/api/report/versions/:fromId/diff/:toId", asyncH(async (req, res) => {
    ...
    if (!from || !to) throw Object.assign(new Error("报告版本不存在或无权访问"), { statusCode: 404 });
