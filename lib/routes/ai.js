...
import { activeLlm } from "../settings.js";

const asyncH = (fn) => (req, res, next) => Promise.resolve(fn(req, res, next)).catch(next);
const ACTIVE_STATUSES = ["planned", "todo", "in_progress"];
...
export function register(app, ctx) {
  ...
  app.post("/api/ai/parse", asyncH(async (req, res) => {
    ...
    const { content } = await chatCompletion({
      ...
      apiKey: llm.apiKey,
      model: llm.model,
      messages: parseTasksPrompt(text, todayString()),
      jsonMode: true,
      timeoutMs: 90000
    });
    ...
    for (const raw of rawTasks.slice(0, 12)) {
      ...
      try {
        const tags = Array.isArray(raw.tags)
          ...
          : [];
        const status = ACTIVE_STATUSES.includes(raw.suggestedStatus) ? raw.suggestedStatus : "todo";
        tasks.push({
