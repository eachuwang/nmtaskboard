...
export function register(app, ctx) {
  ...
  app.put("/api/projects/:id", asyncH(async (req, res) => {
    ...
    const state = await load(req);
    const project = currentProject(state, req.params.id);
    const updated = normalizeProject({ ...project, ...req.body, id: project.id, createdAt: project.createdAt, createdByIdentityId: project.createdByIdentityId, updatedAt: new Date().toISOString() });
