...
export function register(app, ctx) {
  ...
  app.post("/api/report/summary", asyncH(async (req, res) => {
    ...
    const { evidence, options } = await reportEvidence(req, type, start, end, includeCompleted);
    res.json({ type, start, end, timeZone: options.timeZone, evidence, summary: evidence.summary });
  }));
  ...
  app.post("/api/report/template", asyncH(async (req, res) => {
    const { type, start, end, includeCompleted } = parseTypeRange(req.body);
    const { evidence, options } = await reportEvidence(req, type, start, end, includeCompleted);
    res.json({ type, start, end, timeZone: options.timeZone, evidence, summary: evidence.summary, report: templateForType(evidence, type, start, end) });
  }));
