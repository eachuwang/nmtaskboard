...
export function register(app, ctx) {
  ...
  const reportOptions = (includeCompleted) => ({
    includeCompleted,
    timeZone: normalizeSettings(settingsStore(ctx.config).read()).reportTimeZone
  });

  app.post("/api/report/summary", (req, res) => {
    const { type, start, end, includeCompleted } = parseTypeRange(req.body);
    const { tasks } = store.read();
    const options = reportOptions(includeCompleted);
