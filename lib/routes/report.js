import { templateForType, REPORT_TYPES } from "../report.js";
import { buildReportEvidenceBundle } from "../report-evidence.js";
import { chatCompletion } from "../llm.js";
import { polishPrompt, optimizePrompt } from "../prompts.js";
import { extractFacts, validateFactInvariants } from "../report-facts.js";
import { normalizeSettings, normalizeReportTimeZone, resolveActiveLlm } from "../settings.js";
import { progressRecordsForViewer, readableTasks } from "../permissions.js";

const asyncH = (fn) => (req, res, next) => Promise.resolve(fn(req, res, next)).catch(next);

function sse(res) {
  res.writeHead(200, {
    "Content-Type": "text/event-stream; charset=utf-8",
    "Cache-Control": "no-cache, no-transform",
    Connection: "keep-alive",
    "X-Accel-Buffering": "no"
  });
  res.write(": connected\n\n");
  return {
    send(event, data) {
      res.write("event: " + event + "\ndata: " + JSON.stringify(data) + "\n\n");
    }
  };
}

async function llmSettings(ctx, requestContext) {
  const data = normalizeSettings(await ctx.persistence.settings.load(requestContext));
  try {
    return resolveActiveLlm(data);
  } catch (e) {
    throw Object.assign(new Error(e.message + "（请先在设置中配置模型）"), { statusCode: 400 });
  }
}

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

function validateRange(body) {
  const { start, end } = body?.range || {};
  if (!DATE_RE.test(start || "") || !DATE_RE.test(end || "")) {
    throw Object.assign(new Error("日期范围不合法"), { statusCode: 400 });
  }
  if (start > end) throw Object.assign(new Error("开始日期不能晚于结束日期"), { statusCode: 400 });
  return { start, end };
}

// 解析类型与范围：缺省 weekly；handover 无范围
function parseTypeRange(body) {
  const type = typeof body?.type === "string" && REPORT_TYPES.includes(body.type) ? body.type : "weekly";
  if (type === "handover") {
    return { type, start: null, end: null, includeCompleted: !!body?.includeCompleted };
  }
  const { start, end } = validateRange(body);
  return { type, start, end };
}

export function register(app, ctx) {
  const reportOptions = async (requestContext, includeCompleted) => {
    const settings = normalizeSettings(await ctx.persistence.settings.load(requestContext));
    const workspace = requestContext.workspace;
    const timeZone = workspace.type === "team" && workspace.timeZone
      ? normalizeReportTimeZone(workspace.timeZone)
      : settings.reportTimeZone;
    return { includeCompleted, timeZone, includeProgressRecords: true };
  };

  const reportEvidence = async (req, type, start, end, includeCompleted) => {
    const loaded = await ctx.persistence.tasks.load(req.context);
    const readable = readableTasks(req.context, loaded);
    const memberScope = req.context.workspace.type === "team" && req.context.workspace.role === "member";
    const scoped = memberScope
      ? readable.filter((task) => task.taskType === "execution" && task.assigneeIdentityId === req.context.actor.id && task.assignmentStatus !== "removed")
      : readable;
    const tasks = scoped.map((task) => ({ ...task, progressRecords: progressRecordsForViewer(req.context, task) }));
    const options = await reportOptions(req.context, includeCompleted);
    const subject = req.context.workspace.type === "team" ? "team" : "personal";
    const evidence = buildReportEvidenceBundle(tasks, type, start, end, {
      ...options,
      scope: {
        workspaceId: req.context.workspace.id,
        workspaceType: req.context.workspace.type,
        subject,
        role: req.context.workspace.role || "owner",
        actorIdentityId: req.context.actor.id
      },
      ...(memberScope ? { scopeDiagnostic: { code: "restricted_to_own_execution", reason: "团队成员报告仅包含本人负责的执行任务和本人进展记录" } } : {})
    });
    return { evidence, options, subject };
  };

  app.post("/api/report/summary", asyncH(async (req, res) => {
    const { type, start, end, includeCompleted } = parseTypeRange(req.body);
    const { evidence, options, subject } = await reportEvidence(req, type, start, end, includeCompleted);
    res.json({ type, start, end, subject, timeZone: options.timeZone, evidence, summary: evidence.summary });
  }));

  // 模板版周报（无 Key 时的回退产物；票 08 复用）
  app.post("/api/report/template", asyncH(async (req, res) => {
    const { type, start, end, includeCompleted } = parseTypeRange(req.body);
    const { evidence, options, subject } = await reportEvidence(req, type, start, end, includeCompleted);
    res.json({ type, start, end, subject, timeZone: options.timeZone, evidence, summary: evidence.summary, report: templateForType(evidence, type, start, end) });
  }));

  registerReportAi(app, ctx);
}

export function registerReportAi(app, ctx) {
  // AI 优化（SSE 流式；基于证据重组表达，结束后校验事实不变量，违反则保留原稿）
  app.post("/api/report/polish", asyncH(async (req, res) => {
    const draft = typeof req.body?.draft === "string" ? req.body.draft.trim() : "";
    if (!draft) throw Object.assign(new Error("没有可优化的内容"), { statusCode: 400 });
    if (draft.length > 20000) throw Object.assign(new Error("内容过长（最多 20000 字）"), { statusCode: 400 });
    const type = typeof req.body?.type === "string" && REPORT_TYPES.includes(req.body.type) ? req.body.type : "weekly";
    const evidence = req.body?.evidence && typeof req.body.evidence === "object" ? req.body.evidence : null;
    const llm = await llmSettings(ctx, req.context);
    const s = sse(res);
    const ctrl = new AbortController();
    res.on("close", () => ctrl.abort());
    const facts = evidence ? extractFacts(evidence) : null;
    let optimized = "";
    try {
      await chatCompletion({
        baseUrl: llm.baseUrl,
        apiKey: llm.apiKey,
        model: llm.model,
        messages: evidence ? optimizePrompt(draft, facts, type) : polishPrompt(draft, type),
        stream: true,
        timeoutMs: 300000,
        onDelta: (text) => {
          optimized += text;
          s.send("delta", { text });
        },
        signal: ctrl.signal
      });
      if (facts) {
        const check = validateFactInvariants(facts, optimized);
        if (!check.ok) {
          s.send("error", { message: "AI 输出违反事实不变量（任务标题/日期/数量被改写或删除），已保留原稿", violations: check.violations });
          res.end();
          return;
        }
      }
      s.send("done", { model: llm.model });
    } catch (err) {
      s.send("error", { message: err.message || "优化失败" });
    }
    res.end();
  }));
}
