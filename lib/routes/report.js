...
export function registerReportAi(app, ctx) {
  ...
  app.post("/api/report/polish", asyncH(async (req, res) => {
    ...
    try {
      ...
      if (err?.code === "aborted") { res.end(); return; } // 客户端已断开：不写误导性错误事件
  ...
  app.post("/api/report/fill", asyncH(async (req, res) => {
    ...
    const llm = await llmSettings(ctx, req.context);
    const s = sse(res);
    // 首个事件带回任务清单/时区：左侧筛选列表与生成同源，不再依赖单独的 template 预取
    s.send("meta", { summary: generated.evidence.summary, timeZone: generated.timeZone, subject: generated.subject });
    const ctrl = new AbortController();
    res.on("close", () => ctrl.abort());
