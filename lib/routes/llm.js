import { chatCompletion, normalizeOpenAiBaseUrl } from "../llm.js";
...
export function register(app, ctx) {
  ...
  app.post("/api/llm/test", asyncH(async (req, res) => {
    ...
    if (!data.providers.length) {
      throw Object.assign(new Error("尚未配置 LLM 模型，请到超管台「LLM配置」完成配置"), { statusCode: 400 });
    }
    ...
    res.json({ ok: true, message: String(content ?? "").slice(0, 100), latencyMs: Date.now() - started, model: model.id });
  }));
  ...
  app.get("/api/llm/models", asyncH(async (req, res) => {
    ...
    const url = normalizeOpenAiBaseUrl(p.baseUrl) + "/models";
