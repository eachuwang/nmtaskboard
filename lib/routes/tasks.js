...
import { normalizeTask, createTask, applyStatusTransition, STATUSES } from "../tasks.js";
...
export function register(app, ctx) {
  ...
  app.post("/api/tasks", (req, res) => {
    const ts = store.read();
    const task = createTask(req.body || {}, ts.tasks);
    ts.tasks.push(task);
    store.write(ts);
  ...
  app.post("/api/tasks/batch", (req, res) => {
    ...
    const created = items.map((i) => {
      const task = createTask(i, ts.tasks);
      ts.tasks.push(task);
      return task;
    });
    store.write(ts);
  ...
  app.put("/api/tasks/:id", (req, res) => {
    ...
    applyStatusTransition(task, nextStatus);
  ...
  app.post("/api/tasks/reorder", (req, res) => {
    ...
    for (const move of moves) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        applyStatusTransition(task, move.status);
        task.order = i;
        task.updatedAt = new Date().toISOString();
      }
