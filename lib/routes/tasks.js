...
function preventSelfUnassign(task, actorIdentityId, nextIds) {
  const ownerId = task.ownerIdentityId || task.creatorIdentityId || null;
  if (ownerId !== actorIdentityId && assigneesOf(task).includes(actorIdentityId) && !nextIds.includes(actorIdentityId)) {
    throw Object.assign(new Error("负责人不能取消自己，只能调整其他成员"), { statusCode: 403, code: "TASK_SELF_UNASSIGN_FORBIDDEN" });
  }
}
...
export function register(app, ctx) {
  ...
  app.post("/api/tasks/:id/assign", asyncH(async (req, res) => {
    ...
    preventSelfUnassign(task, actorId(req), suppliedIds);
  ...
  app.put("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    if (touchesAssign) {
      requireTaskAction(req.context, { ...task, participantIdentityIds: derivedParticipantIds(task, tasks) }, "assign");
      preventSelfUnassign(task, actorId(req), input.assigneeIdentityIds || []);
    }
    ...
    if (touchesOwner) {
      res.locals.auditSummary = { taskTitle: task.title, changedFields: ["ownerIdentityId"] };
      task.history = [...(task.history || []), { id: `history-${Date.now()}`, action: "owner_transferred", actor: actorName(req), at: new Date().toISOString(), toOwner: task.ownerIdentityId }];
    }
