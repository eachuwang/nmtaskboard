...
function serializableTask(task, tasks, directories = {}, actorId = "") {
  ...
  return {
    ...
    participantDisplayNames: (task.participantIdentityIds || []).map((identityId) => directories.members?.get(identityId)?.displayName || identityId),
...
export function register(app, ctx) {
  ...
  app.get("/api/tasks", asyncH(async (req, res) => {
    ...
    res.json({ tasks: visible.map((task) => serializableTask(task, tasks, directories, actorId(req))) });
  }));
  ...
  app.put("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    const EDIT_FIELDS = ["title", "description", "priority", "dueDate", "tags", "projectId", "stage", "parentTaskId", "participantIdentityIds"];
    ...
    if (input.participantIdentityIds !== undefined && next.participantIdentityIds.length) {
      const memberIds = await workspaceMemberIds(ctx, req);
      for (const identityId of next.participantIdentityIds) {
        if (!memberIds.has(identityId)) throw Object.assign(new Error("参与人必须是当前工作区成员"), { statusCode: 400, code: "TASK_PARTICIPANT_INVALID" });
      }
    }
    ...
    Object.assign(task, {
      ...
      assigneeIdentityId: next.assigneeIdentityId, participantIdentityIds: next.participantIdentityIds
    });
