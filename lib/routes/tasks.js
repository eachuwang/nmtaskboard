...
export function register(app, ctx) {
  ...
  app.post("/api/tasks/:id/assign", asyncH(async (req, res) => {
    ...
    if (!beforeAssignees.length && suppliedIds.length && task.status === "backlog") applyStatusTransition(task, "todo", { prevStatus: "backlog", actor: actorName(req) });
    else if (beforeAssignees.length && !suppliedIds.length && task.status === "todo") applyStatusTransition(task, "backlog", { prevStatus: "todo", actor: actorName(req) });
  ...
  app.put("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    if (assigneeChanged && input.status === undefined) {
      if (!previousAssignees.length && next.assigneeIdentityIds.length && task.status === "backlog") applyStatusTransition(task, "todo", { prevStatus: "backlog", actor: actorName(req) });
      else if (previousAssignees.length && !next.assigneeIdentityIds.length && task.status === "todo") applyStatusTransition(task, "backlog", { prevStatus: "todo", actor: actorName(req) });
    }
