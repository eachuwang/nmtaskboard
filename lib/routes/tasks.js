...
import { readableTasks, requireTaskAction, requireWorkspaceManagement, taskAccess, workspaceCapabilities } from "../permissions.js";
...
export function register(app, ctx) {
  ...
  app.get("/api/tasks", asyncH(async (req, res) => {
    ...
    const visible = readableTasks(req.context, tasks);
    res.json({
  ...
  app.post("/api/tasks/:id/assign", asyncH(async (req, res) => {
    ...
    const identityIds = [...new Set((Array.isArray(req.body?.identityIds) ? req.body.identityIds : []).filter((id) => typeof id === "string" && id.trim()).map((id) => id.trim()))];
    if (!identityIds.length) return res.status(400).json({ error: "请至少选择一名团队成员" });
    ...
    const result = await ctx.persistence.tasks.assign(req.context, req.params.id, identityIds, req.headers["x-action-source"] === "agent" ? "agent" : "ui");
    res.status(result.createdCount ? 201 : 200).json(result);
  }));
  ...
  app.put("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    const nonStatusFields = Object.keys(input).filter((key) => !["status", "reason", "blockReason", "actor"].includes(key));
