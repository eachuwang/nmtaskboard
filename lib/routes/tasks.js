...
import { validateDescription } from "../../shared/rich-description.js";
...
function serializableTask(task, tasks, directories = {}, actorId = "") {
  ...
  return {
    ...
    ownerIdentityId: task.ownerIdentityId || task.creatorIdentityId || null,
    ...(task.ownerIdentityId || task.creatorIdentityId ? { ownerDisplayName: directories.members?.get(task.ownerIdentityId || task.creatorIdentityId)?.displayName || (task.ownerIdentityId || task.creatorIdentityId) } : {}),
...
export function register(app, ctx) {
  ...
  app.post("/api/tasks", asyncH(async (req, res) => {
    ...
    if (input.description !== undefined) {
      const checked = validateDescription(input.description, []);
      if (!checked.valid) return res.status(400).json({ error: checked.issues.find((issue) => issue.severity === "error")?.message || "描述格式无效", code: "DESCRIPTION_INVALID", issues: checked.issues });
    }
  ...
  app.post("/api/tasks/batch", asyncH(async (req, res) => {
    ...
    for (const item of items) {
      if (item.description !== undefined) {
        const checked = validateDescription(item.description, []);
        if (!checked.valid) return res.status(400).json({ error: checked.issues.find((issue) => issue.severity === "error")?.message || "描述格式无效", code: "DESCRIPTION_INVALID", issues: checked.issues });
      }
  ...
  app.put("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    const descriptionDraftId = typeof input.descriptionDraftId === "string" ? input.descriptionDraftId.slice(0, 100) : "";
    const stagedAttachmentIds = Array.isArray(input.stagedAttachmentIds) ? [...new Set(input.stagedAttachmentIds.filter((id) => typeof id === "string"))].slice(0, 20) : [];
    const removedAttachmentIds = Array.isArray(input.removedAttachmentIds) ? [...new Set(input.removedAttachmentIds.filter((id) => typeof id === "string"))].slice(0, 20) : [];
    const stagedAttachments = descriptionDraftId && ctx.persistence.richDescriptions
      ? await ctx.persistence.richDescriptions.staged(req.context, task.id, descriptionDraftId)
      : [];
    if (stagedAttachmentIds.some((id) => !stagedAttachments.some((item) => item.id === id))) return res.status(400).json({ error: "暂存附件无效或已经过期", code: "ATTACHMENT_STAGE_INVALID" });
    if (removedAttachmentIds.some((id) => !(task.attachments || []).some((item) => item.id === id))) return res.status(400).json({ error: "待删除附件不存在", code: "ATTACHMENT_REMOVE_INVALID" });
    const nextAttachments = [
      ...(task.attachments || []).filter((item) => !removedAttachmentIds.includes(item.id)),
      ...stagedAttachments.filter((item) => stagedAttachmentIds.includes(item.id)).map(({ workspaceId, taskId, state, draftId, expiresAt, removedAt, ...item }) => item)
    ];
    if (input.description !== undefined) {
      const checked = validateDescription(input.description, nextAttachments.map((item) => item.id));
      if (!checked.valid) return res.status(400).json({ error: checked.issues.find((issue) => issue.severity === "error")?.message || "描述格式无效", code: "DESCRIPTION_INVALID", issues: checked.issues });
    }
    ...
    if (touchesGrants && (task.ownerIdentityId || task.creatorIdentityId || null) !== actorId(req)) return res.status(403).json({ error: "仅任务所有者可以调整卡片成员权限", code: "TASK_GRANT_FORBIDDEN" });
    // 所有权转移：仅当前所有者，且只能转给现任负责人
    const touchesOwner = input.ownerIdentityId !== undefined && input.ownerIdentityId !== (task.ownerIdentityId || task.creatorIdentityId || null);
    if (touchesOwner) {
      if ((task.ownerIdentityId || task.creatorIdentityId || null) !== actorId(req)) return res.status(403).json({ error: "仅任务所有者可以转移所有权", code: "TASK_OWNER_FORBIDDEN" });
      if (!assigneesOf(task).includes(input.ownerIdentityId)) return res.status(400).json({ error: "所有权只能转移给当前负责人", code: "TASK_OWNER_NOT_ASSIGNEE" });
    }
    ...
    if (input.expectedUpdatedAt && input.expectedUpdatedAt !== task.updatedAt) {
      const descriptionConflict = input.description !== undefined && input.description !== task.description;
      return res.status(409).json({
        error: descriptionConflict ? "任务描述已被其他成员更新，请合并后重试" : "任务已被其他操作更新，请刷新后重试",
        code: descriptionConflict ? "TASK_DESCRIPTION_CONFLICT" : "TASK_VERSION_CONFLICT",
        ...(descriptionConflict ? { latestDescription: task.description, latestUpdatedAt: task.updatedAt } : {})
      });
    }
    ...
    // 转移所有权默认退出负责人：请求未显式保留旧所有者时，转移后自动移出负责人集合
    // （显式保留 = input.assigneeIdentityIds 明确包含旧所有者，即「被设置为负责人」）
    let transferExitsOwner = false;
    if (touchesOwner) {
      const previousOwnerId = task.ownerIdentityId || task.creatorIdentityId || null;
      const explicitlyKept = Array.isArray(input.assigneeIdentityIds) && input.assigneeIdentityIds.includes(previousOwnerId);
      if (previousOwnerId && !explicitlyKept && (next.assigneeIdentityIds || []).includes(previousOwnerId)) {
        next.assigneeIdentityIds = next.assigneeIdentityIds.filter((id) => id !== previousOwnerId);
        next.assigneeIdentityId = next.assigneeIdentityIds[0] || null;
        transferExitsOwner = true;
      }
    }
    ...
    Object.assign(task, {
      title: next.title, description: next.description, descriptionText: next.descriptionText, priority: next.priority, tags: next.tags,
      ...
      memberGrants: next.memberGrants, ownerIdentityId: next.ownerIdentityId
    });
    task.attachments = nextAttachments;
    if (touchesOwner) {
      res.locals.auditSummary = { taskTitle: task.title, changedFields: transferExitsOwner ? ["ownerIdentityId", "assigneeIdentityIds"] : ["ownerIdentityId"] };
      task.history = [...(task.history || []), { id: `history-${Date.now()}`, action: "owner_transferred", actor: actorName(req), at: new Date().toISOString(), toOwner: task.ownerIdentityId, ...(transferExitsOwner ? { exitedOwner: true } : {}) }];
    }
    ...
    const assigneeChanged = touchesAssign || transferExitsOwner;
    ...
    task.updatedAt = new Date().toISOString();
    const descriptionChanged = beforeDescription !== task.description;
    if (ctx.persistence.richDescriptions && (descriptionChanged || stagedAttachmentIds.length || removedAttachmentIds.length)) {
      const committed = await ctx.persistence.richDescriptions.commit(req.context, {
        tasks, taskId: task.id, beforeMarkdown: beforeDescription, afterMarkdown: task.description,
        source: ["manual", "ai", "import", "restore"].includes(input.descriptionSource) ? input.descriptionSource : "manual",
        draftId: descriptionDraftId, stagedIds: stagedAttachmentIds, removedIds: removedAttachmentIds, expectedUpdatedAt: input.expectedUpdatedAt || null
      });
      await Promise.allSettled((committed.purgeObjectKeys || []).map((key) => ctx.objectStore.remove(key)));
      publishWorkspaceEvent(req.context?.workspace?.id, { type: "tasks", at: new Date().toISOString(), actorId: req.context?.actor?.id || "" });
    } else {
      await save(req, tasks);
    }
    ...
    if (task.status !== previousStatus) res.locals.auditSummary = { ...res.locals.auditSummary, statusFrom: previousStatus, statusTo: task.status };
  ...
  app.delete("/api/tasks/:id", asyncH(async (req, res) => {
    ...
    if (ctx.persistence.richDescriptions) {
      const objectKeys = await ctx.persistence.richDescriptions.deleteTask(req.context, task.id);
      await Promise.allSettled([...(task.attachments || []).map((item) => item.objectKey), ...objectKeys].filter(Boolean).map((key) => ctx.objectStore.remove(key)));
    }
    res.locals.auditSummary = { taskTitle: task.title };
