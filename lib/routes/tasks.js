import {
  ...
  calibrateTask, cancelParentTask, createProgressRecord, updateProgressRecord, deleteProgressRecord,
...
export function register(app, ctx) {
  ...
  app.post("/api/tasks", asyncH(async (req, res) => {
    ...
    task.creatorIdentityId = req.context.actor.id;
  ...
  app.post("/api/tasks/reorder", asyncH(async (req, res) => {
    ...
    for (const move of moves) {
      ...
      for (const id of move.orderedIds) {
        ...
        if (task.taskType === "parent" && task.status !== move.status && move.status !== "cancelled") {
          throw Object.assign(new Error("父任务状态由成员任务汇总，只能直接移至已取消"), { statusCode: 400 });
        }
    ...
    for (const move of moves) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        if (task.taskType === "parent" && task.status !== move.status) {
          cancelParentTask(task, ts.tasks, { actor: actorFrom(req), reason });
        } else {
