...
6. 子任务应以父任务条目下的缩进子项（markdown 嵌套列表）呈现，保持父子结构。
7. 结合任务的描述与评论（进展记录）做简洁明了的事实性总结；不得夸大、编造或改变结论。
8. 输出必须保持四个固定一级分节：- Highlights（已完成任务标题）、- Details（已完成任务及其描述/评论摘要）、- In-progress（实施中任务及其摘要）、- Plan for next week（待实施任务标题）；名称与顺序不变，空节保留标题。
