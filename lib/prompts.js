...
export function skeletonReportPrompt(evidence, skeleton, type) {
  const label = type === "handover" ? "离职交接报告" : "工作报告";
  ...
  const digest = JSON.stringify(sectionTrees);
  const system = `你是${label}撰写助手。严格按用户给定的模板骨架结构生成报告，把看板里的真实任务内容填进骨架对应位置。

每条任务用一句话归纳 comments（评论及回复）和 progressRecords（进展记录）中的实际进展，保留关键结果和变化；没有实际进展才描述当前状态。按时间结合上下文，不把问题、建议或计划写成已完成事实。有优先级时自然提及。
