...
export function createAuthService({ repository, bootstrapToken, credentialEncryptionKey = "", sessionTtlMs = 12 * 60 * 60 * 1000, secureCookies = false, fetchImpl, oidcAuthorityBase }) {
  ...
  return {
    ...
    async completeOidcLogin(input) {
      const identity = await oidc.completeLogin(input);
      if (identity.disabledAt) throw new AuthError("ACCOUNT_DISABLED", "账号已停用，请联系系统管理员", 403);
      return issueSession(identity);
    },
    authenticate,
