...
// 显示名称输入校验：1–40 字符，去除首尾空格，禁止纯空白与换行
export function normalizeDisplayNameInput(value) {
  const displayName = typeof value === "string" ? value.trim() : "";
  if (displayName.length < 1 || displayName.length > 40 || /[\r\n]/.test(displayName)) {
    throw new AuthError("DISPLAY_NAME_INVALID", "显示名称需为 1–40 个字符，且不能为纯空白或包含换行", 400);
  }
  return displayName;
}
...
function isCollaborativeWorkspace(workspace) {
  return Boolean(workspace?.id) && workspace.type !== "system" && workspace.type !== "pending";
}

function requireCollaborativeWorkspace(context) {
  if (!isCollaborativeWorkspace(context?.workspace)) {
    throw new AuthError("WORKSPACE_REQUIRED", "请先进入工作区", 409);
  }
}
...
function sessionContext(identity, selectedWorkspace = null) {
  ...
  return Object.freeze({
    actor: Object.freeze({
      ...
      avatarImage: identity.avatarImage || null
    }),
    workspace: Object.freeze({
      id: workspace.id,
      type: workspace.type,
      name: workspace.name || null,
      role: workspace.role || (workspace.type === "system" ? "owner" : "member"),
      visibilityScope: workspace.visibilityScope || "team",
      operationScope: workspace.operationScope || "assigned",
      timeZone: workspace.timeZone || null,
      slug: workspace.slug || workspace.identifier || null,
      taskPrefix: workspace.taskPrefix || workspace.task_prefix || workspace.slug || workspace.identifier || null,
...
export function createAuthService({ repository, audit, sessionTtlMs = 12 * 60 * 60 * 1000, secureCookies = false }) {
  ...
  async function issueSession(identity, ttlMs = sessionTtlMs) {
    ...
    return { identity, expiresAt, cookie: cookie(token, ttlMs) };
  }

  // 记住我：会话延长到 30 天；否则默认 12 小时
  const REMEMBER_TTL_MS = 30 * 24 * 60 * 60 * 1000;
  const sessionTtlFor = (input) => (input?.remember === true ? REMEMBER_TTL_MS : sessionTtlMs);
  ...
  return {
    async login(input) {
      ...
      try {
        ...
        if (identity.reviewStatus === "pending") {
          const result = await issueSession(identity, sessionTtlFor(input));
          const pendingContext = sessionContext(identity);
          await appendAudit(audit, { actor: pendingContext.actor, workspace: null, source: "ui", action: "auth.login", target: { type: "identity", id: identity.id }, outcome: "success", summary: { provider: "local", code: "PENDING_REVIEW" } });
          return result;
        }
        ...
        await appendAudit(audit, { actor: context.actor, workspace: context.workspace, source: "ui", action: "auth.login", target: { type: "identity", id: identity.id }, summary: { provider: "local", remember: input?.remember === true } });
        return result;
      } catch (error) {
        const auditContext = identity ? sessionContext(identity) : null;
        const auditWorkspace = identity && (identity.isSystemAdmin || identity.reviewStatus === "approved") ? auditContext.workspace : null;
        await appendAudit(audit, { actor: auditContext?.actor || null, workspace: auditWorkspace, source: "ui", action: "auth.login", target: { type: "identity", id: identity?.id || null }, outcome: "denied", summary: { provider: "local", code: error.code || "LOGIN_FAILED" } });
        throw error;
      }
    },
    ...
    async updateAvatar(req, input = {}) {
      const context = await authenticate(req);
      if (typeof repository.updateAvatar !== "function") throw new AuthError("AVATAR_UNSUPPORTED", "当前存储不支持头像", 400);
      const avatarImage = typeof input.avatarImage === "string" ? input.avatarImage.trim() : "";
      if (avatarImage && (!/^data:image\/(png|jpeg|webp|svg\+xml);base64,/.test(avatarImage) || avatarImage.length > 400_000)) {
        throw new AuthError("AVATAR_INVALID", "头像格式不支持或文件过大", 400);
      }
      await repository.updateAvatar(context.actor.id, avatarImage || null);
      await appendAudit(audit, { actor: context.actor, workspace: context.workspace, source: "ui", action: "auth.avatar_change", target: { type: "identity", id: context.actor.id }, summary: {} });
      return { ok: true, avatarImage: avatarImage || null };
    },
    // 显示名称与登录用户名分离：仅本人可改，所有工作区共用，历史记录保留发生时名称
    async updateProfile(req, input = {}) {
      const context = await authenticate(req);
      if (typeof repository.updateDisplayName !== "function") throw new AuthError("PROFILE_UNSUPPORTED", "当前存储不支持修改显示名称", 400);
      const displayName = normalizeDisplayNameInput(input.displayName);
      await repository.updateDisplayName(context.actor.id, displayName);
      await appendAudit(audit, { actor: { ...context.actor, displayName }, workspace: context.workspace, source: "ui", action: "identity.display_name_update", target: { type: "identity", id: context.actor.id }, summary: {} });
      return { ok: true, actor: { ...context.actor, displayName } };
    },
    ...
    async createTeam(req, input, requestId) {
      const context = await authenticate(req);
      if (context.actor.isSystemAdmin) throw new AuthError("SYSTEM_ADMIN_NO_WORKSPACE", "系统管理员不能创建工作区", 403);
      const name = typeof input.name === "string" ? input.name.trim() : "";
      const generatedIdentifier = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 32);
      const identifier = (typeof input.identifier === "string" ? input.identifier.trim().toLowerCase() : "")
        || generatedIdentifier
        || `workspace-${crypto.randomUUID().slice(0, 8)}`;
      const timeZone = typeof input.timeZone === "string" && input.timeZone.trim() ? input.timeZone.trim() : "Asia/Shanghai";
      if (name.length < 2 || name.length > 50) throw new AuthError("WORKSPACE_NAME_INVALID", "工作区名称需要 2–50 个字符", 400);
      if (!/^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])$/.test(identifier)) {
        throw new AuthError("WORKSPACE_SLUG_INVALID", "工作区标识需要 2–32 位小写字母、数字或连字符", 400);
      }
      if (!validTimeZone(timeZone)) throw new AuthError("WORKSPACE_TIME_ZONE_INVALID", "请选择有效的工作区时区", 400);
    ...
    async deleteTeam(req, input = {}) {
      const context = await authenticate(req);
      if (context.actor.isSystemAdmin) throw new AuthError("SYSTEM_ADMIN_NO_WORKSPACE", "系统管理员不能删除工作区", 403);
      requireCollaborativeWorkspace(context);
      const confirmName = typeof input.confirmName === "string" ? input.confirmName : "";
      const token = parseCookies(req.headers.cookie)[COOKIE_NAME];
      const tokenHash = crypto.createHash("sha256").update(token).digest("hex");
      const result = await repository.deleteTeam(context.actor.id, context.workspace.id, confirmName, tokenHash);
      await appendAudit(audit, {
        actor: context.actor, workspace: context.workspace, source: "ui",
        action: "workspace.delete", target: { type: "workspace", id: context.workspace.id }, summary: {}
      });
      return result;
    },
    async teamMembers(context) {
      ...
      const invitationResult = manager && repository.listOutgoingInvitations
        ? await repository.listOutgoingInvitations(context.actor.id, context.workspace.id)
        : { pending: [], history: [] };
      const invitations = invitationResult.pending || [];
      const invitationHistory = invitationResult.history || [];
      const recentEvents = manager && audit?.list ? await audit.list(context, { limit: 12 }) : [];
      return { actorId: context.actor.id, workspace: result.workspace, members: result.members, invitations, invitationHistory, recentEvents };
    },
    ...
    async inviteTeamMember(context, input) {
      ...
      publishNotification(identityId, { type: "invitation.created", invitationId: invitation.id, workspaceId: context.workspace.id });
      return { invitation };
    },
    ...
    async respondToInvitation(context, invitationId, action) {
      ...
      const workspace = { id: result.workspace.id, name: result.workspace.name, type: "workspace", role: decision === "accepted" ? "member" : null };
      await appendAudit(audit, {
        actor: context.actor,
        workspace,
        source: "ui",
    ...
    // 工作区所有者修改成员显示名称：账号级改名（各工作区共用），登录名不变；本人仍可经个人资料自改
    async renameTeamMember(context, identityId, input) {
      requireCollaborativeWorkspace(context);
      if (!identityId) throw new AuthError("MEMBER_REQUIRED", "请选择团队成员", 400);
      if (typeof repository.renameTeamMember !== "function") throw new AuthError("PROFILE_UNSUPPORTED", "当前存储不支持修改显示名称", 400);
      const displayName = normalizeDisplayNameInput(input.displayName);
      const member = await repository.renameTeamMember(context.actor.id, context.workspace.id, identityId, displayName);
      await appendAudit(audit, { actor: context.actor, workspace: context.workspace, source: "ui", action: "workspace.member_rename", target: { type: "identity", id: member.id }, summary: { changedFields: ["displayName"] } });
      return { member };
    },
    ...
    async removeTeamMember(context, identityId, input) {
      ...
      await appendAudit(audit, { actor: context.actor, workspace: context.workspace, source: "ui", action: "workspace.member_remove", target: { type: "identity", id: identityId }, summary: { changedFields: result.handling ? ["membership", "tasks"] : ["membership"] } });
      return result;
    },
    async teamPermissions(context) {
      return { workspaceType: "workspace", actorId: context.actor.id, role: context.workspace.role, visibilityScope: "team", operationScope: "assigned" };
    },
    ...
    async updateTeamTimeZone(context, input) {
      return this.updateWorkspace(context, input);
    },
    async updateWorkspace(context, input = {}) {
      requireCollaborativeWorkspace(context);
      if (!["owner", "admin"].includes(context.workspace.role)) throw new AuthError("WORKSPACE_MANAGE_FORBIDDEN", "只有所有者或管理员可以修改工作区信息", 403);
      const patch = {};
      ...
      if (input.description !== undefined) {
        patch.description = typeof input.description === "string" ? input.description.trim().slice(0, 2000) : "";
      }
      if (input.slug !== undefined || input.identifier !== undefined) {
        const slug = String(input.slug || input.identifier || "").trim().toLowerCase();
        if (!/^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])$/.test(slug)) {
          throw new AuthError("WORKSPACE_SLUG_INVALID", "工作区标识需要 2–32 位小写字母、数字或连字符", 400);
        }
        patch.slug = slug;
        patch.taskPrefix = typeof input.taskPrefix === "string" && input.taskPrefix.trim() ? input.taskPrefix.trim().slice(0, 32) : slug;
      } else if (input.taskPrefix !== undefined) {
        const prefix = String(input.taskPrefix || "").trim().slice(0, 32);
        if (!prefix) throw new AuthError("WORKSPACE_PREFIX_INVALID", "任务前缀不能为空", 400);
        patch.taskPrefix = prefix;
      }
      if (input.timeZone !== undefined) {
        const timeZone = typeof input.timeZone === "string" ? input.timeZone.trim() : "";
        if (!validTimeZone(timeZone)) throw new AuthError("TEAM_TIME_ZONE_INVALID", "请选择有效的工作区时区", 400);
        patch.timeZone = timeZone;
      }
      const workspace = await repository.updateWorkspace(context.actor.id, context.workspace.id, patch);
      await appendAudit(audit, {
        actor: context.actor, workspace: context.workspace, source: "ui",
        action: "workspace.profile_update", target: { type: "workspace", id: workspace.id },
    ...
    async selectWorkspace(req, workspaceId) {
      const context = await authenticate(req);
      if (context.actor.isSystemAdmin) throw new AuthError("SYSTEM_ADMIN_NO_WORKSPACE", "系统管理员不能进入工作区看板", 403);
      const token = parseCookies(req.headers.cookie)[COOKIE_NAME];
      const tokenHash = crypto.createHash("sha256").update(token).digest("hex");
...
export function registerAuthRoutes(app, auth) {
  ...
  app.post("/api/auth/avatar", asyncH(async (req, res) => res.json(await auth.updateAvatar(req, req.body || {}))));
  app.post("/api/auth/profile", asyncH(async (req, res) => res.json(await auth.updateProfile(req, req.body || {}))));
  ...
  app.delete("/api/workspaces/current", asyncH(async (req, res) => res.json(await auth.deleteTeam(req, req.body || {}))));
  app.patch("/api/workspaces/current", asyncH(async (req, res) => res.json(await auth.updateWorkspace(req.context, req.body || {}))));
  ...
  app.patch("/api/team/members/:identityId/display-name", asyncH(async (req, res) => res.json(await auth.renameTeamMember(req.context, req.params.identityId, req.body || {}))));
