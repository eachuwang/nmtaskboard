...
export function createAuthService({ repository, audit, bootstrapToken, credentialEncryptionKey = "", sessionTtlMs = 12 * 60 * 60 * 1000, secureCookies = false, fetchImpl, oidcAuthorityBase }) {
  ...
  return {
    ...
    async teamMembers(context) {
      ...
      return { actorId: context.actor.id, workspace: result.workspace, members: result.members };
    },
    ...
    async teamPermissions(context) {
      if (context.workspace.type !== "team") return { actorId: context.actor.id, role: "owner", visibilityScope: "team", operationScope: "assigned" };
      return { actorId: context.actor.id, role: context.workspace.role, visibilityScope: context.workspace.visibilityScope, operationScope: context.workspace.operationScope };
    },
