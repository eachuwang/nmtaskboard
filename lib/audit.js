...
const WRITE_ACTIONS = [
  ["PUT", /^\/api\/status-workflow$/, "workspace.status_workflow", "workspace"],
  ["POST", /^\/api\/tasks$/, "task.create", "task"],
  ["POST", /^\/api\/tasks\/batch$/, "task.batch_create", "task"],
  ["POST", /^\/api\/tasks\/trash\/([^/]+)\/restore$/, "task.restore", "task"],
  ["DELETE", /^\/api\/tasks\/trash\/([^/]+)$/, "task.purge", "task"],
  ...
  ["POST", /^\/api\/connections$/, "connection.create", "connection"],
  ["POST", /^\/api\/connections\/([^/]+)\/reauthorize$/, "connection.reauthorize", "connection"],
  ["DELETE", /^\/api\/connections\/([^/]+)$/, "connection.disconnect", "connection"],
  ["POST", /^\/api\/repositories$/, "repository.create", "repository"],
  ["POST", /^\/api\/projects\/([^/]+)\/repository-bindings$/, "project.resource_bind", "project"],
  ["POST", /^\/api\/tasks\/([^/]+)\/attachments$/, "attachment.create", "attachment"],
...
function safeSummary(summary) {
  ...
  for (const [key, value] of Object.entries(summary)) {
    // 拖拽轨迹：仅保留任务标题与状态迁移（枚举值），不夹带其他内容
    if (key === "moves" && Array.isArray(value)) {
      safe.moves = value.slice(0, 20).map((move) => ({
        title: String(move?.title || ""),
        statusFrom: String(move?.statusFrom || ""),
        statusTo: String(move?.statusTo || "")
      }));
      continue;
    }
    if (!["method", "statusCode", "count", "code", "provider", "changedFields", "runId", "turnId", "toolCallId", "statusFrom", "statusTo", "taskTitle"].includes(key)) continue;
...
export function attachAuditTrail(audit) {
  return (req, res, next) => {
    ...
    res.once("finish", () => {
      const body = req.body;
      // 记录本次提交触及的字段名（如状态/截止时间/描述），供“最近操作”展示具体变更内容。
      const changedFields = (req.method === "PUT" || req.method === "PATCH") && body && typeof body === "object" && !Array.isArray(body)
        ? Object.keys(body).filter((key) => key !== "actionSource").slice(0, 20)
        : [];
      appendAudit(audit, {
        ...
        summary: { method: req.method, statusCode: res.statusCode, ...(changedFields.length ? { changedFields } : {}), ...(res.locals?.auditSummary || {}) }
      }).catch(() => {});
    });
    next();
  };
