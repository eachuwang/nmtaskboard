...
export async function createReportEvidence(ctx, context, { type, start, end, includeCompleted = false, scope }) {
  ...
  // 全量任务标题：报告树按父任务聚合时，父可能被范围过滤掉但仍需其标题作组头
  const taskTitlesById = new Map(readable.map((task) => [task.id, task.title]));
  const evidence = buildReportEvidenceBundle(tasks, type, start, end, {
    workflow: await ctx.persistence.statusWorkflow?.load(context),
    includeCompleted,
    timeZone,
...
export function filterReportEvidence(evidence, excludedTaskIds = [], includeNextWeek = true) {
  ...
  return {
    ...
    summary: {
      ...source,
      sections,
      ...(source.statusGroups ? { statusGroups: source.statusGroups.map((group) => ({ ...group, items: group.items.filter((item) => !excluded.has(item.id)) })) } : {}),
