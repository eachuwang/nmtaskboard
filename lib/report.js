...
import { templateForReportType } from "../shared/report-template.js";
...
function taskReportLines(task, level = 0, meta = "") {
  ...
  const description = excerptText(task.descriptionText || descriptionToText(task.description), 120);
...
// 旧确定性实现（保留作字节兼容测试参照；正式入口 templateForType 在下方委托 renderReport）
export function legacyTemplateForType(summary, type, start, end) {
...
// 正式入口：返回该类型的模板骨架文本（无 LLM 时即为草稿；有 LLM 时由 /api/report/fill 填实）
export function templateForType(summary, type, start, end) {
  return templateForReportType(type).text;
}
