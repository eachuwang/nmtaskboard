...
const STATUS_SET = new Set(["planned", "todo", "in_progress", "blocked", "done", "cancelled"]);
const INITIAL_STATUS_SET = new Set(["planned", "todo"]);
const TRANSITIONS = {
  planned: new Set(["todo", "cancelled"]),
...
const trustedSnapshotAt = (task, end, timeZone) => {
  ...
  if (statusEntries.some((entry) => (
    !HISTORY_ACTION_SET.has(entry?.action) ||
  ...
  for (let index = replayIndex; index < entries.length; index++) {
    ...
    if (entry.action !== "moved" || entry.fromStatus !== status || !TRANSITIONS[status]?.has(entry.toStatus)) {
      return invalidTimeline("invalid_transition", "存在非法状态跳转");
    }
    if (REASON_REQUIRED.has(`${status}>${entry.toStatus}`) && (typeof entry.reason !== "string" || !entry.reason.trim())) {
  ...
  if (!hasLaterEntry && task.status !== status) return invalidTimeline("status_mismatch", "最后轨迹与卡片状态不一致");
...
export function buildReportSummary(tasks, start, end, opts = {}) {
  ...
  const inProgress = reportable.filter(({ snapshot }) => snapshot.status === "in_progress");
  const used = new Set([...completed, ...blocked, ...inProgress].map(({ task }) => task.id));
  const created = reportable.filter(({ task, snapshot }) => (
    !used.has(task.id) &&
    (snapshot.status === "planned" || snapshot.status === "todo") &&
  ...
  const nextWeek = reportable
    .filter(
      ({ task, snapshot }) =>
        !used.has(task.id) &&
        (snapshot.status === "planned" || snapshot.status === "todo") &&
        ((task.dueDate && task.dueDate >= nextStart && task.dueDate <= nextEnd) || task.priority === "high")
    )
