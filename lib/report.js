...
export function templateReport(summary, start, end) {
  if (summary.statusGroups) return workflowTemplate(summary, "weekly", start, end);
  const lines = [];
  ...
  if (completed.length) {
    ...
    pushTaskGroup(lines, completed, (t) => `（完成于 ${mdDay(t.completedDay || t.completedAt)}）`);
    lines.push("");
  }
  if (inProgress.length) {
    ...
    lines.push("");
    lines.push("以下工作仍在推进：");
    pushTaskGroup(lines, inProgress);
    lines.push("");
  }
  if (blocked.length) {
    lines.push("## 风险与阻塞");
    ...
    pushTaskGroup(lines, blocked, (t) => (t.blockReason ? `（阻塞原因：${t.blockReason}）` : ""));
    lines.push("");
  }
  if (created.length) {
    lines.push("## 本周新增");
  ...
  if (summary.nextWeek.length) {
    ...
    lines.push("");
    lines.push("下周重点关注：");
    pushTaskGroup(lines, summary.nextWeek, (t) => (t.dueDate ? "（截止 " + mdDay(t.dueDate) + "）" : "（高优先级）"));
  }
  return lines.join("\n");
}
...
export function templateForType(summary, type, start, end) {
  ...
  if (summary.statusGroups) return workflowTemplate(summary, type, start, end);
  ...
  if (type === "weekly") {
    ...
    const section = (label, items, fmt) => {
      ...
      lines.push(`- **${label}**`);
      const { roots, childrenOf } = groupReportItems(items);
      for (const root of roots) pushTaskLines(lines, root, fmt, 1, childrenOf);
      lines.push("");
    };
    section("Highlights", completed, () => "");
    section("Details", completed, (t) => (showDate ? "（完成于 " + mdDay(t.completedDay || t.completedAt) + "）" : ""));
    section("In-progress", inProgressAll, (t) => (t.blockReason ? "（阻塞原因：" + t.blockReason + "）" : ""));
    section("Plan for next week", summary.nextWeek || [], (t) => (t.dueDate ? "（截止 " + mdDay(t.dueDate) + "）" : ""));
    return lines.join("\n");
  }
