...
function tasksAdapter(pool, schema) {
  return {
    async load(context) {
      ...
      return result.rows.map((row) => recordsByTask.has(row.payload.id)
        ? { ...row.payload, progressRecords: recordsByTask.get(row.payload.id) }
        : row.payload);
    },
    ...
    async requestCancellation(context, executionTaskId, reason) {
      return transaction(pool, schema, async (client) => {
        ...
        if (!execution || execution.taskType !== "execution") throw teamError("TASK_NOT_FOUND", "任务不存在", 404);
        ...
        if (!parentResult.rows[0] || parentResult.rows[0].payload.status === "cancelled") throw teamError("CANCEL_REQUEST_INVALID", "父任务已取消，不能重复提交取消申请", 409);
    ...
    async decideCancellation(context, requestId, decision, decisionReason, expectedUpdatedAt = null) {
      return transaction(pool, schema, async (client) => {
        ...
        const parent = parentResult.rows[0]?.payload;
        if (!parent || parent.taskType !== "parent") throw teamError("TASK_NOT_FOUND", "父任务不存在", 404);
        ...
        const executions = executionRows.rows.map((row) => row.payload).filter((task) => task.taskType === "execution" && task.parentTaskId === parent.id && task.assignmentStatus !== "removed");
        const cancelled = [];
