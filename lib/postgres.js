...
function authAdapter(pool, schema) {
  ...
  return {
    ...
    async createPendingRegistration({ login, displayName, passwordHash }) {
      return transaction(pool, schema, async (client) => {
        const duplicate = await client.query(
          "SELECT id FROM identities WHERE lower(login_name) = lower($1) OR lower(email) = lower($1)",
          [login]
        );
        if (duplicate.rows.length) {
    ...
    async listDirectoryUsers(search = "") {
      const needle = String(search || "").trim();
      const result = await query(pool, schema, `
        SELECT id, display_name, email, login_name, approved_at, created_at
        FROM identities
        WHERE review_status = 'approved' AND is_system_admin = false
          AND ($1 = '' OR display_name ILIKE '%' || $1 || '%' OR login_name ILIKE '%' || $1 || '%' OR COALESCE(email, '') ILIKE '%' || $1 || '%')
    ...
    async approveRegistration(id) {
      return transaction(pool, schema, async (client) => {
        const pending = await client.query(
          "SELECT * FROM identities WHERE id = $1 AND review_status = 'pending' FOR UPDATE",
          [id]
        );
        if (!pending.rows[0]) {
        ...
        const updated = await client.query(`
          UPDATE identities SET review_status = 'approved', approved_at = now(), updated_at = now()
          WHERE id = $1 RETURNING *
        `, [id]);
        const identity = updated.rows[0];
        const personalWorkspaceId = `personal-${identity.id}`;
        await client.query(`
          INSERT INTO workspaces (id, type, name, created_by_identity_id)
          VALUES ($1, 'personal', $2, $3)
        `, [personalWorkspaceId, `${identity.display_name}的个人空间`, identity.id]);
        await client.query(`
          INSERT INTO workspace_members (workspace_id, identity_id, role)
          VALUES ($1, $2, 'owner')
        `, [personalWorkspaceId, identity.id]);
        return identityFrom(identity);
      });
    },
    async rejectRegistration(id) {
      return transaction(pool, schema, async (client) => {
        const pending = await client.query(
          "SELECT id FROM identities WHERE id = $1 AND review_status = 'pending' FOR UPDATE",
          [id]
        );
        if (!pending.rows[0]) {
          throw Object.assign(new Error("待审记录不存在"), { code: "REGISTRATION_NOT_FOUND", statusCode: 404 });
