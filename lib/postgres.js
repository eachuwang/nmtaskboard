    ...
    async listTeamMembers(actorId, workspaceId) {
      return transaction(pool, schema, async (client) => {
        ...
        const result = await client.query(`
          ...
          GROUP BY i.id, i.display_name, i.email, i.login_name, m.role, m.created_at
    ...
    async inviteTeamMember(actorId, workspaceId, identifier) {
      return transaction(pool, schema, async (client) => {
        ...
        if (membership.rows[0]) {
          await client.query(`
            UPDATE workspace_members SET role = 'member', removed_at = NULL, removed_by_identity_id = NULL,
        ...
        return memberFrom({ ...target, role: "member", created_at: new Date().toISOString(), unfinished_task_count: 0 });
      });
    },
    async changeTeamMemberRole(actorId, workspaceId, identityId, role) {
      return transaction(pool, schema, async (client) => {
        ...
        const updated = await client.query(`
          ...
          RETURNING i.id, i.display_name, i.email, i.login_name, m.role, m.created_at
