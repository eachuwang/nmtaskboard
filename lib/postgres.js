...
async function replaceTasks(client, current, tasks, { memberExecutionOnly = false } = {}) {
  ...
  for (const [ordinal, task] of nextTasks.entries()) {
    ...
    if (task.taskType === "execution" && task.assigneeIdentityId && task.parentTaskId) {
      await client.query(`
      ...
      `, [current.workspaceId, task.id, task.assigneeIdentityId, task.assignees?.[0] || task.assigneeIdentityId, task.status, JSON.stringify({ parentTaskId: task.parentTaskId }), timestamp(task.updatedAt)]);
    }
  }
}
...
function tasksAdapter(pool, schema) {
  return {
    ...
    async assign(context, parentTaskId, identityIds, source = "ui") {
      return transaction(pool, schema, async (client) => {
        ...
        if (!parent || parent.taskType !== "parent") throw Object.assign(new Error("团队父任务不存在"), { statusCode: 404 });
        ...
        `, [current.workspaceId, identityIds]);
        ...
        for (const member of members.rows) {
          let execution = pending.find((task) => task.taskType === "execution" && task.parentTaskId === parentTaskId && task.assigneeIdentityId === member.id);
          if (!execution) {
        ...
        const participantMap = new Map((parent.participants || []).map((item) => [item.identityId, item]));
        for (const execution of executions) participantMap.set(execution.assigneeIdentityId, { identityId: execution.assigneeIdentityId, displayName: execution.assignees[0], status: execution.status, executionTaskId: execution.id });
        parent.participants = [...participantMap.values()];
        ...
        return { parent, executions, createdCount };
      });
    }
  };
}
