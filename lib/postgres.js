...
import { applyStatusTransition, cancelParentTask, createTask, ensureTaskExtras, normalizeTransitionReason, recordHistory } from "./tasks.js";
...
function tasksAdapter(pool, schema) {
  return {
    ...
    async assign(context, parentTaskId, identityIds, source = "ui", expectedUpdatedAt = null, auditEvent = null) {
  ...
  return {
    ...
    async createLocalUser({ login, displayName, passwordHash, mustChangePassword = false }) {
      return transaction(pool, schema, async (client) => {
        ...
        await client.query(`
          INSERT INTO workspaces (id, type, name, created_by_identity_id)
          VALUES ($1, 'personal', $2, $3)
        `, [personalWorkspaceId, `${displayName}的个人空间`, identityId]);
        await client.query(`
          INSERT INTO workspace_members (workspace_id, identity_id, role)
          VALUES ($1, $2, 'owner')
        `, [personalWorkspaceId, identityId]);
    ...
    async listDirectoryUsers(search = "") {
      ...
      const result = await query(pool, schema, `
        ...
        LEFT JOIN workspaces w ON w.id = m.workspace_id AND w.type = 'team'
      ...
      return transaction(pool, schema, async (client) => {
        ...
        if (status === "approved") {
          await client.query(`
            ...
            VALUES ($1, 'personal', $2, $3)
            ON CONFLICT (id) DO NOTHING
          `, [`personal-${id}`, `${identity.display_name}的个人空间`, id]);
          await client.query(`
            INSERT INTO workspace_members (workspace_id, identity_id, role)
            VALUES ($1, $2, 'owner')
            ON CONFLICT (workspace_id, identity_id) DO NOTHING
    ...
    async cancelIdentity(id, { usernameHash, emailHash }) {
      return transaction(pool, schema, async (client) => {
        ...
        const owners = await client.query(`
          ...
          WHERE w.type = 'team' AND m.role = 'owner' AND m.removed_at IS NULL
          FOR UPDATE OF w
        `, [id]);
        ...
        await client.query("UPDATE workspaces SET created_by_identity_id = (SELECT identity_id FROM workspace_members WHERE workspace_id = workspaces.id AND role = 'owner' AND removed_at IS NULL LIMIT 1) WHERE type = 'team' AND created_by_identity_id = $1", [id]);
        await client.query("DELETE FROM workspaces WHERE type = 'personal' AND created_by_identity_id = $1", [id]);
    ...
    async listOutgoingInvitations(actorId, workspaceId) {
      return transaction(pool, schema, async (client) => {
        ...
        const result = await client.query(`
          ...
          WHERE invitation.workspace_id = $1 AND invitation.status = 'pending'
    ...
    async listIncomingInvitations(identityId) {
      const result = await query(pool, schema, `
        ...
        WHERE invitation.invitee_identity_id = $1 AND invitation.status = 'pending'
        ORDER BY invitation.created_at DESC
      `, [identityId]);
      return result.rows.map(invitationFrom);
    },
    async resolveTeamInvitation(identityId, invitationId, decision) {
      return transaction(pool, schema, async (client) => {
        const found = await client.query(`
          ...
          WHERE invitation.id = $1 AND invitation.invitee_identity_id = $2 AND invitation.status = 'pending'
