...
async function lockStatusWorkflow(client, workspaceId) {
  await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`status-workflow:${workspaceId}`]);
}
async function readStatusWorkflow(client, workspaceId) {
  const result = await client.query("SELECT payload FROM status_workflows WHERE workspace_id = $1", [workspaceId]);
  return result.rows[0]?.payload || defaultWorkflow();
}
async function writeStatusWorkflow(client, workspaceId, workflow) {
  await client.query("INSERT INTO status_workflows (workspace_id, payload) VALUES ($1, $2::jsonb) ON CONFLICT (workspace_id) DO UPDATE SET payload = EXCLUDED.payload, updated_at = now()", [workspaceId, JSON.stringify(workflow)]);
}
function statusWorkflowAdapter(pool, schema) {
  return {
    async load(context) {
      const { workspaceId } = postgresContext(context);
      const result = await query(pool, schema, "SELECT payload FROM status_workflows WHERE workspace_id = $1", [workspaceId]);
      return result.rows[0]?.payload || defaultWorkflow();
    },
    async save(context, input) {
      return transaction(pool, schema, async (client) => {
        const current = await ensureContext(client, context);
        await lockStatusWorkflow(client, current.workspaceId);
        const workflow = await readStatusWorkflow(client, current.workspaceId);
        const stored = await client.query("SELECT payload FROM tasks WHERE workspace_id = $1 ORDER BY ordinal FOR UPDATE", [current.workspaceId]);
        const tasks = stored.rows.map((row) => withStatusDefinition(row.payload, workflow));
        const result = applyWorkflow(workflow, tasks, input, current.actorName);
        await writeStatusWorkflow(client, current.workspaceId, result.workflow);
        await replaceTasks(client, current, result.tasks);
        return result;
      });
    }
  };
...
async function replaceTasks(client, current, tasks, { memberExecutionOnly = false } = {}) {
  ...
  for (const task of tasks) {
    if (workflow.revision && task.statusRevision !== workflow.revision) throw workflowError("状态方案已变化，请刷新后重试", 409);
  }
  ...
  for (const [ordinal, rawTask] of nextTasks.entries()) {
    const task = normalizeTask(rawTask, workflow);
  ...
  return {
    async load(context) {
      ...
      return snapshot(pool, schema, async (client) => {
        const workflow = await readStatusWorkflow(client, workspaceId);
        const result = await client.query("SELECT payload FROM tasks WHERE workspace_id = $1 ORDER BY ordinal", [workspaceId]);
        return result.rows.map((row) => ensureTaskExtras(withStatusDefinition(row.payload, workflow)));
      });
    },
...
function descriptionVersionFrom(row) {
  return {
    id: row.id,
    taskId: row.task_id,
...
function richDescriptionsAdapter(pool, schema) {
  return {
    async stage(context, input) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `
        INSERT INTO task_attachments (
          id, workspace_id, task_id, object_key, filename, content_type, size_bytes,
          created_by_identity_id, state, draft_id, expires_at
        ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'staged',$9,now() + interval '24 hours')
        RETURNING *
      `, [input.id, current.workspaceId, input.taskId, input.objectKey, input.filename, input.contentType, input.size, current.actorId, input.draftId]);
      return attachmentFrom(result.rows[0]);
    },
    async staged(context, taskId, draftId) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `SELECT * FROM task_attachments WHERE workspace_id=$1 AND task_id=$2 AND draft_id=$3 AND created_by_identity_id=$4 AND state='staged' ORDER BY created_at`, [current.workspaceId, taskId, draftId, current.actorId]);
      return result.rows.map(attachmentFrom);
    },
    async expired(context) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `DELETE FROM task_attachments WHERE workspace_id=$1 AND state='staged' AND expires_at <= now() RETURNING *`, [current.workspaceId]);
      return result.rows.map(attachmentFrom);
    },
    async discard(context, taskId, draftId) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `DELETE FROM task_attachments WHERE workspace_id=$1 AND task_id=$2 AND draft_id=$3 AND created_by_identity_id=$4 AND state='staged' RETURNING *`, [current.workspaceId, taskId, draftId, current.actorId]);
      return result.rows.map(attachmentFrom);
    },
    async find(context, attachmentId) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `SELECT * FROM task_attachments WHERE workspace_id=$1 AND id=$2 AND (state IN ('active','retained') OR (state='staged' AND created_by_identity_id=$3 AND expires_at > now()))`, [current.workspaceId, attachmentId, current.actorId]);
      return result.rows[0] ? attachmentFrom(result.rows[0]) : null;
    },
    async commit(context, { tasks, taskId, beforeMarkdown, afterMarkdown, source = "manual", draftId, stagedIds = [], removedIds = [], expectedUpdatedAt }) {
      return transaction(pool, schema, async (client) => {
        const current = await ensureContext(client, context);
        await lockStatusWorkflow(client, current.workspaceId);
        const stored = await client.query("SELECT payload FROM tasks WHERE workspace_id=$1 AND id=$2 FOR UPDATE", [current.workspaceId, taskId]);
        if (!stored.rows[0]) throw teamError("TASK_NOT_FOUND", "任务不存在", 404);
        if (expectedUpdatedAt && stored.rows[0].payload.updatedAt !== expectedUpdatedAt) throw teamError("TASK_DESCRIPTION_CONFLICT", "任务已被其他成员更新，请合并描述后重试", 409);
        const latest = await client.query("SELECT payload FROM tasks WHERE workspace_id=$1 ORDER BY ordinal", [current.workspaceId]);
        const updated = tasks.find((task) => task.id === taskId);
        if (!updated) throw teamError("TASK_NOT_FOUND", "任务不存在", 404);
        await replaceTasks(client, current, latest.rows.map((row) => row.payload.id === taskId ? updated : row.payload));
        if (beforeMarkdown !== afterMarkdown) {
          const max = await client.query("SELECT COALESCE(MAX(revision),0)::int revision FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2", [current.workspaceId, taskId]);
          let revision = max.rows[0].revision;
          if (!revision && beforeMarkdown) {
            revision += 1;
            await client.query(`INSERT INTO task_description_versions (id,workspace_id,task_id,revision,markdown,source,actor_identity_id,actor_display_name) VALUES ($1,$2,$3,$4,$5,'created',$6,$7)`, [crypto.randomUUID(), current.workspaceId, taskId, revision, beforeMarkdown, current.actorId, current.actorName]);
          }
          revision += 1;
          await client.query(`INSERT INTO task_description_versions (id,workspace_id,task_id,revision,markdown,source,actor_identity_id,actor_display_name) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [crypto.randomUUID(), current.workspaceId, taskId, revision, afterMarkdown, source, current.actorId, current.actorName]);
          await client.query(`DELETE FROM task_description_versions WHERE id IN (SELECT id FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2 ORDER BY revision DESC OFFSET 50)`, [current.workspaceId, taskId]);
        }
        if (stagedIds.length) await client.query(`UPDATE task_attachments SET state='active',draft_id=NULL,expires_at=NULL WHERE workspace_id=$1 AND task_id=$2 AND draft_id=$3 AND created_by_identity_id=$4 AND id=ANY($5::text[])`, [current.workspaceId, taskId, draftId, current.actorId, stagedIds]);
        if (removedIds.length) await client.query(`UPDATE task_attachments SET state='retained',removed_at=now() WHERE workspace_id=$1 AND task_id=$2 AND id=ANY($3::text[])`, [current.workspaceId, taskId, removedIds]);
        const savedVersions = await client.query("SELECT markdown FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2", [current.workspaceId, taskId]);
        const referenced = [...new Set([afterMarkdown, ...savedVersions.rows.map((row) => row.markdown)].flatMap(attachmentReferences))];
        const purged = await client.query(`DELETE FROM task_attachments WHERE workspace_id=$1 AND task_id=$2 AND state='retained' AND NOT (id=ANY($3::text[])) RETURNING object_key`, [current.workspaceId, taskId, referenced]);
        return { ok: true, purgeObjectKeys: purged.rows.map((row) => row.object_key) };
      });
    },
    async versions(context, taskId) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `SELECT * FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2 ORDER BY revision DESC LIMIT 50`, [current.workspaceId, taskId]);
      return result.rows.map(({ markdown, ...row }) => descriptionVersionFrom(row));
    },
    async version(context, taskId, versionId) {
      const current = postgresContext(context);
      const result = await query(pool, schema, `SELECT * FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2 AND id=$3`, [current.workspaceId, taskId, versionId]);
      return result.rows[0] ? descriptionVersionFrom(result.rows[0]) : null;
    },
    async deleteTask(context, taskId) {
      const current = postgresContext(context);
      return transaction(pool, schema, async (client) => {
        await client.query("DELETE FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2", [current.workspaceId, taskId]);
        const removed = await client.query("DELETE FROM task_attachments WHERE workspace_id=$1 AND task_id=$2 RETURNING object_key", [current.workspaceId, taskId]);
        return removed.rows.map((row) => row.object_key);
      });
    },
    async removeAttachment(context, taskId, attachment) {
      const current = postgresContext(context);
      const versions = await query(pool, schema, "SELECT markdown FROM task_description_versions WHERE workspace_id=$1 AND task_id=$2", [current.workspaceId, taskId]);
      const retained = versions.rows.some((row) => attachmentReferences(row.markdown).includes(attachment.id));
      if (retained) {
        await query(pool, schema, "UPDATE task_attachments SET state='retained',removed_at=now() WHERE workspace_id=$1 AND task_id=$2 AND id=$3", [current.workspaceId, taskId, attachment.id]);
        return [];
      }
      const removed = await query(pool, schema, "DELETE FROM task_attachments WHERE workspace_id=$1 AND task_id=$2 AND id=$3 RETURNING object_key", [current.workspaceId, taskId, attachment.id]);
      return removed.rows.map((row) => row.object_key);
    }
  };
}
...
// 周报模板：owner_identity_id NULL=工作区级（需管理权），非空=个人级
function reportTemplatesAdapter(pool, schema) {
  return {
    async list(context) {
      const { workspaceId, actorId } = postgresContext(context);
      const result = await query(pool, schema, `
        SELECT * FROM report_templates
        WHERE workspace_id = $1 AND (owner_identity_id IS NULL OR owner_identity_id = $2)
        ORDER BY owner_identity_id NULLS FIRST, created_at
      `, [workspaceId, actorId]);
      return result.rows;
    },
    async save(context, template, ownerIdentityId) {
      const { workspaceId } = postgresContext(context);
      const templateType = template.type === "handover" ? "handover" : "time";
      const payload = JSON.stringify({ text: template.text || "" });
      const result = await query(pool, schema, `
        INSERT INTO report_templates (id, workspace_id, owner_identity_id, name, template_type, payload, is_default)
        VALUES ($1, $2, $3, $4, $5, $6::jsonb, false)
        ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, template_type = EXCLUDED.template_type, payload = EXCLUDED.payload, updated_at = now()
        RETURNING *
      `, [template.id, workspaceId, ownerIdentityId, template.name, templateType, payload]);
      return result.rows[0];
    },
    async remove(context, id, ownerIdentityId) {
      const { workspaceId } = postgresContext(context);
      await query(pool, schema, `
        DELETE FROM report_templates WHERE id = $1 AND workspace_id = $2 AND owner_identity_id IS NOT DISTINCT FROM $3
      `, [id, workspaceId, ownerIdentityId]);
    },
    async setDefault(context, id, templateType, ownerIdentityId) {
      const { workspaceId } = postgresContext(context);
      await transaction(pool, schema, async (client) => {
        await ensureContext(client, context);
        await client.query(`UPDATE report_templates SET is_default = false WHERE workspace_id = $1 AND owner_identity_id IS NOT DISTINCT FROM $2 AND template_type = $3 AND is_default = true`, [workspaceId, ownerIdentityId, templateType]);
        await client.query(`UPDATE report_templates SET is_default = true WHERE id = $1 AND workspace_id = $2`, [id, workspaceId]);
      });
    }
  };
}
  ...
  return {
    async export(context) {
      ...
      return snapshot(pool, schema, async (client) => {
        ...
        const descriptionVersions = await client.query("SELECT * FROM task_description_versions WHERE workspace_id = $1 ORDER BY task_id, revision", [workspaceId]);
        return {
          tasks: tasks.rows.map((row) => row.payload),
          descriptionVersions: descriptionVersions.rows.map(descriptionVersionFrom),
          statusWorkflow: await readStatusWorkflow(client, workspaceId),
          reportTemplates: (await client.query("SELECT id, owner_identity_id, name, template_type, payload, is_default, created_at FROM report_templates WHERE workspace_id = $1 ORDER BY created_at", [workspaceId])).rows,
    ...
    async replace(context, data) {
      await transaction(pool, schema, async (client) => {
        const current = await ensureContext(client, context);
        await lockStatusWorkflow(client, current.workspaceId);
        await writeStatusWorkflow(client, current.workspaceId, data.statusWorkflow || defaultWorkflow());
        ...
        if (Array.isArray(data.descriptionVersions)) {
          await client.query("DELETE FROM task_description_versions WHERE workspace_id=$1", [current.workspaceId]);
          for (const version of data.descriptionVersions) {
            if (!(data.tasks || []).some((task) => task.id === version.taskId) || typeof version.markdown !== "string") continue;
            await client.query(`INSERT INTO task_description_versions (id,workspace_id,task_id,revision,markdown,source,actor_identity_id,actor_display_name,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [version.id || crypto.randomUUID(), current.workspaceId, version.taskId, version.revision, version.markdown, ["manual","ai","import","restore","created"].includes(version.source) ? version.source : "import", version.actorIdentityId || null, version.actorDisplayName || current.actorName, timestamp(version.createdAt) || new Date().toISOString()]);
          }
        }
        if (data.settings) await replaceSettings(client, current, data.settings);
        if (Array.isArray(data.reportTemplates)) {
          await client.query("DELETE FROM report_templates WHERE workspace_id = $1", [current.workspaceId]);
          for (const t of data.reportTemplates) {
            if (!t || !t.id || !t.payload) continue;
            await client.query(`INSERT INTO report_templates (id, workspace_id, owner_identity_id, name, template_type, payload, is_default, created_at) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7,COALESCE($8::timestamptz, now())) ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, template_type=EXCLUDED.template_type, payload=EXCLUDED.payload, is_default=EXCLUDED.is_default, updated_at=now()`, [t.id, current.workspaceId, t.ownerIdentityId || t.owner_identity_id || null, t.name, t.templateType || t.template_type || "time", JSON.stringify(t.payload), Boolean(t.isDefault ?? t.is_default), t.createdAt || t.created_at]);
          }
        }
      });
    }
  ...
  return {
    ...
    async ensureBuiltInAdmin({ login, displayName, passwordHash, mustChangePassword }) {
      return transaction(pool, schema, async (client) => {
        ...
        const inserted = await client.query(`
          INSERT INTO identities (id, display_name, login_name, password_hash, is_system_admin, must_change_password, review_status, username)
          VALUES ($1, $2, $3, $4, true, $5, 'approved', $3)
          RETURNING *
    ...
    async createLocalUser({ login, displayName, passwordHash, mustChangePassword = false }) {
      const username = login && !login.includes("@") ? login : displayName;
      return transaction(pool, schema, async (client) => {
        const duplicate = await client.query("SELECT id FROM identities WHERE lower(login_name) = lower($1)", [login]);
        if (duplicate.rows.length) {
          throw Object.assign(new Error("登录名已存在"), { code: "LOGIN_EXISTS", statusCode: 409 });
        ...
        const inserted = await client.query(`
          INSERT INTO identities (id, display_name, login_name, email, password_hash, is_system_admin, must_change_password, review_status, approved_at, username)
          VALUES ($1, $2, $3, $3, $4, false, $5, 'approved', now(), $6)
          RETURNING *
        `, [identityId, displayName, login, passwordHash, mustChangePassword === true, username]);
        const personalWorkspaceId = `workspace-${identityId}`;
        const workspaceSlug = `workspace-${identityId.slice(0, 8)}`;
        await client.query(`
          INSERT INTO workspaces (id, type, name, slug, task_prefix, created_by_identity_id)
    ...
    async createPendingRegistration({ login, displayName, passwordHash, usernameHash, emailHash }) {
      return transaction(pool, schema, async (client) => {
        ...
        const identityId = crypto.randomUUID();
        const inserted = await client.query(`
          INSERT INTO identities (id, display_name, login_name, email, password_hash, is_system_admin, must_change_password, review_status, username)
          VALUES ($1, $2, $3, $3, $4, false, false, 'pending', $2)
          RETURNING *
    ...
    async listTeamMembers(actorId, workspaceId) {
      return transaction(pool, schema, async (client) => {
        ...
        const result = await client.query(`
          SELECT i.id, i.display_name, i.email, i.login_name, i.avatar_image, i.username, m.role, m.visibility_scope, m.operation_scope, m.created_at,
          ...
          LEFT JOIN LATERAL (
            SELECT
              count(*) FILTER (WHERE COALESCE(t.payload->'statusDefinition'->>'lifecycle', CASE WHEN t.status IN ('done', 'cancelled') THEN 'terminal' ELSE 'pending' END) <> 'terminal')::int AS unfinished_task_count,
              count(*) FILTER (WHERE t.status = 'backlog')::int AS backlog_count,
              count(*) FILTER (WHERE t.status = 'todo' OR (t.payload->'statusDefinition'->>'lifecycle' = 'pending' AND t.status LIKE 'cs_%'))::int AS todo_count,
              count(*) FILTER (WHERE t.status = 'in_progress' OR (t.payload->'statusDefinition'->>'lifecycle' = 'active' AND t.status LIKE 'cs_%'))::int AS in_progress_count,
              count(*) FILTER (WHERE t.status = 'in_review')::int AS in_review_count,
              count(*) FILTER (WHERE t.status = 'blocked' OR t.payload->'statusDefinition'->>'lifecycle' = 'blocked')::int AS blocked_count,
              count(*) FILTER (WHERE t.status = 'done' OR t.payload->'statusDefinition'->>'outcome' = 'completed')::int AS done_count
            FROM tasks t
    ...
    // 工作区所有者修改成员显示名称：更新账号级 display_name（各工作区共用），登录名不变
    async renameTeamMember(actorId, workspaceId, identityId, displayName) {
      return transaction(pool, schema, async (client) => {
        await requireTeamRole(client, workspaceId, actorId, ["owner"]);
        const updated = await client.query(`
          UPDATE identities i
          ...
          RETURNING i.id, i.display_name, i.email, i.login_name, i.avatar_image, i.username, m.role, m.visibility_scope, m.operation_scope, m.created_at
        `, [workspaceId, identityId, displayName]);
        if (!updated.rows[0]) throw teamError("MEMBER_NOT_FOUND", "团队成员不存在", 404);
        return memberFrom({ ...updated.rows[0], unfinished_task_count: 0 });
      });
    },
    ...
    async teamMemberRemovalImpact(actorId, workspaceId, identityId) {
      return transaction(pool, schema, async (client) => {
        ...
        const tasks = await client.query(`
          ...
          FROM tasks t
          WHERE t.workspace_id = $1 AND t.assignee_identity_id = $2 AND COALESCE(t.payload->'statusDefinition'->>'lifecycle', CASE WHEN t.status IN ('done', 'cancelled') THEN 'terminal' ELSE 'pending' END) <> 'terminal'
    ...
    async removeTeamMember(actorId, workspaceId, identityId, handling) {
      return transaction(pool, schema, async (client) => {
        ...
        const unfinished = await client.query(`
          ...
          WHERE workspace_id = $1 AND assignee_identity_id = $2 AND COALESCE(payload->'statusDefinition'->>'lifecycle', CASE WHEN status IN ('done', 'cancelled') THEN 'terminal' ELSE 'pending' END) <> 'terminal'
        `, [workspaceId, identityId]);
        await client.query(`
          UPDATE tasks
...
function auditAdapter(pool, schema) {
  return {
    ...
    async list(context, { limit = 100 } = {}) {
      ...
      const result = await query(pool, schema, `
        SELECT a.*, t.title AS target_task_title
        FROM audit_events a
...
function auditEventFrom(row) {
  return {
    ...
    targetTitle: row.target_task_title || null,
...
export async function createPostgresPersistence(config) {
  ...
  return {
    ...
    richDescriptions: richDescriptionsAdapter(pool, schema),
    statusWorkflow: statusWorkflowAdapter(pool, schema),
    ...
    reportTemplates: reportTemplatesAdapter(pool, schema),
