...
export function createLocalObjectStore(dataDir) {
  const root = path.join(dataDir, "attachments");
  const file = (key) => {
    const target = path.resolve(root, ...String(key).split("/").filter(Boolean));
    if (!target.startsWith(`${path.resolve(root)}${path.sep}`)) throw storageError("OBJECT_KEY_INVALID", "附件路径无效", 400);
    return target;
  };
  return {
    kind: "local",
    async put({ key, body, contentType }) {
      ...
      fs.writeFileSync(`${target}.json`, JSON.stringify({ contentType: contentType || "application/octet-stream" }), { mode: 0o600 });
      return { key, size: bytes.length };
    },
    async get(key) {
      const target = file(key);
      if (!fs.existsSync(target)) throw storageError("OBJECT_NOT_FOUND", "附件不存在", 404);
      const meta = fs.existsSync(`${target}.json`) ? JSON.parse(fs.readFileSync(`${target}.json`, "utf8")) : {};
      const body = fs.readFileSync(target);
      return { body, size: body.length, contentType: meta.contentType || "application/octet-stream" };
    },
    async remove(key) {
      const target = file(key);
...
export function createS3ObjectStore(config) {
  ...
  return {
    ...
    async put({ key, body, contentType }) {
      ...
      const response = await outboundFetch(signedRequest.url, { method: "PUT", headers: signedRequest.headers, body: bytes });
      if (!response.ok) throw storageError("STORAGE_PUT_FAILED", `对象存储写入失败（HTTP ${response.status}）`);
      return { key, size: bytes.length };
    },
    async get(key) {
      const signedRequest = signed("GET", key, "");
      const response = await outboundFetch(signedRequest.url, { headers: signedRequest.headers });
    ...
    async remove(key) {
      ...
      await outboundFetch(signedRequest.url, { method: "DELETE", headers: signedRequest.headers });
    }
  };
}
...
export function createObjectStore(config = {}, options = {}) {
  ...
  return createLocalObjectStore(config.dataDir || path.join(process.cwd(), "data"));
}
