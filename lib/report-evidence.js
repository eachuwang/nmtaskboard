...
function evidenceItem(item, source, end, timeZone) {
  ...
  return {
    ...item,
    description: source.descriptionText || descriptionToText(source.description),
...
export function buildReportEvidenceBundle(tasks, type, start, end, options = {}) {
  ...
  const evidenceSummary = { ...summary, ...(summary.statusGroups ? { statusGroups: summary.statusGroups.map((group) => ({ ...group, items: group.items.map((item) => evidenceItem(item, byId.get(item.id) || item, end, timeZone)) })) } : {}), sections, ...(summary.nextWeek ? { nextWeek } : {}) };
