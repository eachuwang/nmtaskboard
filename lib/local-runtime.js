...
const WINDOWS_ADMIN_HINT = "本机数据库不能用「管理员」身份运行。请关掉这个窗口，不要选「以管理员身份运行」，用普通方式打开终端或双击 start.cmd。若 Windows 关闭了用户账户控制（UAC），请先打开 UAC，或换一个标准用户。";
...
export function postgresRejectedAdmin(text) {
  return /administrative permissions is not permitted/i.test(String(text || ""));
}

export function windowsIsElevated() {
...
function databaseFailure(stage, error, logs) {
  ...
  if (postgresRejectedAdmin(`${errorText(error)}\n${detail}`)) {
    return new Error(WINDOWS_ADMIN_HINT);
  }
  return new Error(
...
export async function ensureDatabase(config, { EmbeddedPostgres, isElevated = windowsIsElevated } = {}) {
  if (config.databaseUrl) {
    return { ...config, localPostgres: false, stopLocalPostgres: async () => {} };
  }
  if (isElevated()) {
    throw new Error(WINDOWS_ADMIN_HINT);
  }
...
export async function prepareLocalRuntime(config, options = {}) {
  return ensureDatabase(config, options);
}
