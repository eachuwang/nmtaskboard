...
function participantFromTask(task, isViewer = false) {
  return {
    identityId: task.assigneeIdentityId || null,
    displayName: task.assignees?.[0] || task.assigneeIdentityId || "未分派",
    status: task.status,
...
export function projectTaskRelations(context, tasks) {
  ...
  return tasks.map((task) => {
    const ownExecution = task.taskType === "execution" && task.assigneeIdentityId === actorId;
    ...
    const aggregateSource = task.taskType === "parent" ? siblings : [];
    return {
      ...task,
      memberRelation,
      ...
      ...(task.taskType === "parent" ? {
        aggregateStatus: aggregateExecutionStatus(aggregateSource),
