...
// 服务端权限判定是安全边界；返回给前端的 permission 仅用于交互提示，不能替代这里的校验。
export function taskAccess(context, task) {
  ...
  const cancellationLocked = task?.status === "cancelled" && (task?.taskType === "parent" || task?.parentCancelledAt);
  if (["owner", "admin"].includes(context?.workspace?.role)) return { read: true, edit: true, delete: true, changeStatus: !cancellationLocked, addProgress: true, requestCancellation: false, access: "manage" };
  ...
  const parentCreator = task?.taskType === "parent" && task?.creatorIdentityId === context.actor?.id;
  if (parentCreator) return { read: true, edit: false, delete: false, changeStatus: task.status === "planned", addProgress: true, requestCancellation: false, access: "owner" };
...
export function aggregateExecutionStatus(executions = []) {
  ...
  if (statuses.includes("done") && statuses.some((status) => !["done", "cancelled"].includes(status))) return "in_progress";
...
export function projectTaskRelations(context, tasks) {
  ...
  const parentsById = new Map(tasks.filter((task) => task.taskType === "parent").map((task) => [task.id, task]));
  ...
  return tasks.map((task) => {
    ...
    return {
      ...
      participantSummary,
      ...(task.taskType === "execution" && parentsById.has(task.parentTaskId) ? {
        parentStatus: parentsById.get(task.parentTaskId).status,
        parentCancelReason: parentsById.get(task.parentTaskId).cancelReason || null
      } : {}),
