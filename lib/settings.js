...
// 工作区分工角色（与工作区权限独立）。预设角色仅在缺失时惰性填充，首次写入时落库。
export const PRESET_TEAM_ROLES = Object.freeze([
  { id: "role-pm", name: "项目经理" },
  { id: "role-product", name: "产品经理" },
  { id: "role-design", name: "设计师" },
  { id: "role-dev", name: "开发人员" },
  { id: "role-qa", name: "测试人员" },
  { id: "role-ops", name: "运维人员" }
]);

function normalizeTeamRoles(value) {
  if (!Array.isArray(value)) return PRESET_TEAM_ROLES.map((role) => ({ ...role }));
  const roles = [];
  const names = new Set();
  for (const item of value) {
    ...
    names.add(name);
    roles.push({ id, name });
    if (roles.length >= 50) break;
  }
  return roles.length ? roles : PRESET_TEAM_ROLES.map((role) => ({ ...role }));
}

function normalizeMemberRoles(value, roles) {
  const valid = new Set(roles.map((role) => role.id));
  const result = {};
  if (!value || typeof value !== "object" || Array.isArray(value)) return result;
  for (const [identityId, roleIds] of Object.entries(value)) {
    if (!identityId || !Array.isArray(roleIds)) continue;
    const ids = [...new Set(roleIds.filter((id) => valid.has(id)))].slice(0, 20);
    if (ids.length) result[identityId] = ids;
  }
  return result;
}
...
export function normalizeSettings(raw) {
  ...
  data.teamRoles = normalizeTeamRoles(raw?.teamRoles);
  data.memberRoles = normalizeMemberRoles(raw?.memberRoles, data.teamRoles);
...
// 按提供方+模型 id 解析（用于 NM Helper 等处的用户自选模型）；未命中则抛错，防止注入未配置模型
export function resolveLlmById(settings, providerId, modelId) {
  const data = normalizeSettings(settings);
  const p = data.providers.find((x) => x.id === providerId);
  if (!p) throw new LlmError("provider_not_found", "提供方不存在", 404);
  if (!p.baseUrl?.trim()) throw new LlmError("not_configured", "提供方「" + (p.name || p.id) + "」未配置 API 地址");
  const model = p.models.find((m) => m.id === modelId);
  if (!model?.id) throw new LlmError("model_not_found", "该提供方下不存在模型「" + modelId + "」", 404);
  return {
    provider: p,
    baseUrl: p.baseUrl,
    apiKey: p.apiKey || "",
...
// 用户可选模型清单（不含密钥）：id 为 providerId:modelId 复合键
export function listAvailableModels(settings) {
  ...
  for (const p of data.providers) {
    ...
    for (const m of p.models || []) {
      if (!m?.id) continue;
      out.push({ id: p.id + ":" + m.id, providerId: p.id, providerName: p.name || p.id, modelId: m.id, isDefault: p.id === data.defaultProviderId && m.id === (p.defaultModelId || p.models[0]?.id) });
    }
  }
  return out;
}
