...
export async function confirmAgentDraft(ctx, context, draft) {
  ...
  const createdTags = normalized.tags.filter((tag) => tag.action === "create").map((tag) => ({
    name: tag.name, color: tag.color, creator: actor, createdAt: now
  }));
  const pending = [...currentTasks];
  const createdTasks = normalized.tasks.map((input) => {
  ...
  try {
    ...
    await appendAudit(ctx.audit, {
      ...
      outcome,
      summary: originAuditSummary(draft, { count: createdTasks.length, changedFields: createdTags.length ? ["tasks", "tags"] : ["tasks"] })
    }).catch(() => {});
  }
}
