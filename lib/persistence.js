...
function reportTemplatesJsonAdapter(store) {
  const toRow = (t) => ({ id: t.id, owner_identity_id: t.ownerIdentityId ?? null, name: t.name, template_type: t.templateType || "time", payload: t.payload || { text: "" }, is_default: Boolean(t.isDefault), created_at: t.created_at || t.createdAt });
  return {
    async list(context) {
      const actorId = context?.actor?.id;
      const workspaceId = context?.workspace?.id;
      return (store.read().templates || []).filter((t) => t.workspaceId === workspaceId && (t.ownerIdentityId == null || t.ownerIdentityId === actorId)).map(toRow);
    },
    async save(context, template, ownerIdentityId) {
      const data = store.read();
      const templates = (data.templates || []).filter((t) => !(t.id === template.id && t.workspaceId === context.workspace.id));
      const row = { id: template.id, workspaceId: context.workspace.id, ownerIdentityId: ownerIdentityId || null, name: template.name, templateType: template.type === "handover" ? "handover" : "time", payload: { text: template.text || "" }, isDefault: false, created_at: new Date().toISOString() };
      templates.push(row);
    ...
    async remove(context, id, ownerIdentityId) {
      const data = store.read();
      store.write({ templates: (data.templates || []).filter((t) => !(t.id === id && t.workspaceId === context.workspace.id && (t.ownerIdentityId ?? null) === (ownerIdentityId ?? null))) });
    },
    async setDefault(context, id, templateType, ownerIdentityId) {
      const data = store.read();
      const templates = (data.templates || []).map((t) => {
        if (t.workspaceId !== context.workspace.id) return t;
        if ((t.ownerIdentityId ?? null) === (ownerIdentityId ?? null) && (t.templateType || "time") === templateType) t.isDefault = (t.id === id);
        return t;
      });
      store.write({ templates });
    }
...
export function createJsonPersistence(config) {
  ...
  const reportTemplateStore = jsonStore(config.dataDir, "report-templates.json", { templates: [] });
  return {
    driver: "json",
    ...
    reportTemplates: reportTemplatesJsonAdapter(reportTemplateStore),
    backup: {
      async export() {
        return { tasks: taskStore.read().tasks, descriptionVersions: taskStore.read().descriptionVersions || {}, statusWorkflow: taskStore.read().statusWorkflow || defaultWorkflow(), projects: projectStore.read().projects || [], resources: projectStore.read().resources || [], settings: settingStore.read(), reportTemplates: reportTemplateStore.read().templates || [] };
      },
      async replace(context, data) {
        ...
        if (Array.isArray(data.reportTemplates)) reportTemplateStore.write({ templates: data.reportTemplates });
      }
    },
    async health() {
