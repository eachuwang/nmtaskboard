import { statusLabel, activeStatuses } from "../shared/task-statuses.js";
import { descriptionToText } from "../shared/rich-description.js";
import { appendAudit } from "./audit.js";
import { originAuditSummary } from "./agent-protocol.js";
import { readTeamProgress } from "./agent-team-tools.js";
...
// agent 路径草稿：返回所选模板的骨架文本（无 LLM；agent 的回答由 LLM 叙述）
async function draftTeamReportText(ctx, context, evidence, parsed, templateId) {
  const template = await resolveTemplate(ctx.persistence, context, parsed.type, templateId);
...
function compactTask(task, context) {
  return {
    id: task.id,
    title: task.title,
    description: task.descriptionText || descriptionToText(task.description),
    descriptionMarkdown: task.description || "",
    status: task.status,
    statusDefinition: task.statusDefinition,
...
async function runTool(ctx, context, tool, args) {
  ...
  if (tool === "readReport" || tool === "draftTeamReport") {
    ...
    return tool === "draftTeamReport"
      ? { ...result, draft: await draftTeamReportText(ctx, context, evidence, parsed, args.templateId), publicationStatus: "draft" }
      : result;
  }

  const tasks = await visibleState(ctx, context);
  if (tool === "readBoard") return { statuses: activeStatuses(context.statusWorkflow), tasks: tasks.map((task) => compactTask(task, context)), count: tasks.length };
  ...
  if (tool === "readHistory") return { task: compactTask(task, context), history: (task.history || []).map((entry) => ({ ...entry, fromLabel: statusLabel(entry.fromStatus, context.statusWorkflow), toLabel: statusLabel(entry.toStatus, context.statusWorkflow) })) };
