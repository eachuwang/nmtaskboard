import { completionStats } from "../shared/task-statuses.js";
...
export function normalizeProject(input = {}) {
  ...
  return {
    ...
    participantIdentityIds: Array.isArray(input.participantIdentityIds)
      ? [...new Set(input.participantIdentityIds.map((item) => value(item, 100)).filter(Boolean))].slice(0, 50)
      : [],
...
export function withProjectDerived(project, tasks = [], resources = []) {
  ...
  return {
    ...
    eligibleTaskCount: completionStats(projectTasks).total,
    excludedTaskCount: completionStats(projectTasks).excluded,
    completedTaskCount: completionStats(projectTasks).completed,
    progress: projectTasks.length ? completionStats(projectTasks).progress : 0,
