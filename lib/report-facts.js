...
function evidenceItems(evidence) {
  ...
  return [...(summary.statusGroups || []).flatMap((group) => group.items || []), ...SECTION_KEYS.flatMap((key) => summary.sections?.[key] || []), ...(summary.nextWeek || [])];
}
...
function itemValues(item) {
  ...
  return [
    ...
    ...(references.historyEntryIds || []), ...(references.progressRecordIds || []), ...(references.commentIds || []),
    ...(item?.evidence?.history || []).flatMap((entry) => [entry.reason, entry.actor, entry.at]),
    ...(item?.evidence?.comments || []).flatMap((record) => [record.text, record.author, record.createdAt, record.updatedAt]),
...
export function validateFactInvariants(facts, text, options = {}) {
  const requireTitles = options.requireTitles !== false; // 按骨架生成时任务可子集，不强制全出现
  const violations = [];
  if (requireTitles) {
...
// 结构化填充不变量校验：LLM 填的叙事字段里不得出现证据外的日期/数字。
export function validateFactInvariantsJson(facts, llmFilled) {
  const texts = [];
  if (Array.isArray(llmFilled?.summary?.points)) texts.push(...llmFilled.summary.points);
  if (llmFilled?.items && typeof llmFilled.items === "object") {
    for (const fields of Object.values(llmFilled.items)) {
      if (!fields || typeof fields !== "object") continue;
      for (const v of Object.values(fields)) if (typeof v === "string") texts.push(v);
    }
  }
