...
import { DEFAULT_STATUSES, activeStatuses, resolveStatus, taskStatus, completionStats } from "../shared/task-statuses.js";
import { descriptionToText, normalizeDescription } from "../shared/rich-description.js";

// Statuses are categories, not a state machine: any workspace member may move
// an issue directly to any status.
export const STATUSES = DEFAULT_STATUSES.map((s) => s.id);
...
// 参与人派生：本任务子树（任意深度）负责人的并集，排除本任务负责人。
// 显式 participantIdentityIds 字段仅作历史兼容存储，展示与权限一律以派生为准。
export function derivedParticipantIds(task, tasks = []) {
  const own = new Set(assigneesOf(task));
  ...
  const walk = (id) => {
    for (const child of childrenOf.get(id) || []) {
      ...
      for (const identityId of assigneesOf(child)) if (!own.has(identityId)) participants.add(identityId);
      walk(child.id);
    }
  };
  if (task?.id) walk(task.id);
  return [...participants];
}

export const assigneesOf = (task) => Array.isArray(task?.assigneeIdentityIds) && task.assigneeIdentityIds.length ? task.assigneeIdentityIds : (task?.assigneeIdentityId ? [task.assigneeIdentityId] : []);
...
export function normalizeTask(input = {}, workflow) {
  const task = { ...input };
  if (!text(task.title, 200)) throw Object.assign(new Error("任务标题不能为空"), { statusCode: 400 });
  task.title = text(task.title, 200);
  if (workflow) {
    const definition = resolveStatus(task.status, workflow);
    if (task.status && !definition) throw Object.assign(new Error("状态不属于当前方案，请刷新后重试"), { statusCode: 400 });
    task.status = (definition || activeStatuses(workflow)[0]).id;
    task.statusDefinition = definition || activeStatuses(workflow)[0];
    task.statusValue = task.statusDefinition.value;
    task.statusRevision = workflow.revision;
  } else if (!STATUSES.includes(task.status) && (!task.status || task.statusDefinition?.id !== task.status)) task.status = "backlog";
  if (!PRIORITIES.includes(task.priority)) task.priority = "none";
  task.description = normalizeDescription(task.description);
  task.descriptionText = descriptionToText(task.description);
  ...
  task.participantIdentityIds = Array.isArray(task.participantIdentityIds)
    ? [...new Set(task.participantIdentityIds.map((item) => idOrNull(item)).filter(Boolean))].slice(0, 50)
    : [];
  // 多负责人：assigneeIdentityIds 为真相，assigneeIdentityId 仅保留首位作兼容投影
  task.assigneeIdentityIds = Array.isArray(task.assigneeIdentityIds)
    ? [...new Set(task.assigneeIdentityIds.map((item) => idOrNull(item)).filter(Boolean))].slice(0, 50)
    : (task.assigneeIdentityId ? [task.assigneeIdentityId] : []);
  task.assigneeIdentityId = task.assigneeIdentityIds[0] || null;
  // 成员级能力授予：能力白名单 assign/edit/comment；键为身份 id（派生参与人在此不可知，不做集合校验）
  const normalizedGrants = {};
  if (task.memberGrants && typeof task.memberGrants === "object" && !Array.isArray(task.memberGrants)) {
    for (const [key, value] of Object.entries(task.memberGrants)) {
      if (!value || typeof value !== "object" || Array.isArray(value)) continue;
      const grant = {};
      for (const cap of ["assign", "edit", "comment"]) if (typeof value[cap] === "boolean") grant[cap] = value[cap];
      if (Object.keys(grant).length) normalizedGrants[key] = grant;
    }
  }
  task.memberGrants = normalizedGrants;
  ...
  task.blockReason = taskStatus(task)?.lifecycle === "blocked" ? text(task.blockReason, 500) || null : null;
  task.cancelReason = taskStatus(task)?.outcome === "abandoned" ? text(task.cancelReason, 500) || null : null;
  return task;
}

export function validateStatusTransition(fromStatus, toStatus, workflow) {
  if (!resolveStatus(toStatus, workflow)) throw Object.assign(new Error("非法状态"), { statusCode: 400 });
  return true;
}
...
export function projectProgress(tasks = [], projectId) {
  ...
  return completionStats(items).progress;
}
...
export function recordHistory(task, entry) {
  ...
  task.history.push({
    ...
    ...(entry.fromDefinition ? { fromDefinition: entry.fromDefinition } : {}),
    ...(entry.toDefinition || task.statusDefinition ? { toDefinition: entry.toDefinition || task.statusDefinition } : {})
  });
}
...
export function applyStatusTransition(task, newStatus, opts = {}) {
  ...
  const from = opts.previousDefinition || taskStatus(task);
  const to = resolveStatus(newStatus, opts.workflow);
  validateStatusTransition(previous, newStatus, opts.workflow);
  if (previous === to.id && !opts.force) return task;
  const at = opts.effectiveAt || nowIso();
  task.status = to.id;
  task.statusDefinition = { ...to };
  task.statusValue = to.value;
  if (opts.workflow) task.statusRevision = opts.workflow.revision;
  // Built-in mode retains the established timestamps; custom flows keep first start.
  task.startedAt = to.builtin && !opts.source
    ? (to.id === "in_progress" ? at : task.startedAt || null)
    : task.startedAt || (to.lifecycle === "active" ? at : null);
  const endedAt = to.lifecycle === "terminal"
    ? (from?.lifecycle === "terminal" ? task.endedAt || task.completedAt || task.cancelledAt || at : at) : null;
  task.endedAt = endedAt;
  task.completedAt = to.outcome === "completed" ? (from?.outcome === "completed" ? task.completedAt || endedAt : at) : null;
  task.cancelledAt = to.outcome === "abandoned" ? (from?.outcome === "abandoned" ? task.cancelledAt || endedAt : at) : null;
  task.blockReason = to.lifecycle === "blocked" ? normalizeTransitionReason(opts.reason || opts.blockReason) : null;
  task.cancelReason = to.outcome === "abandoned" ? normalizeTransitionReason(opts.reason || opts.cancelReason) : null;
  recordHistory(task, {
    action: "moved", fromStatus: previous, toStatus: to.id, actor: opts.actor,
    fromDefinition: from, toDefinition: to, source: opts.source,
...
export function ensureTaskExtras(task) {
  ...
  if (!STATUSES.includes(task.status) && (!task.status || task.statusDefinition?.id !== task.status)) task.status = "backlog";
  if (!PRIORITIES.includes(task.priority)) task.priority = "none";
  ...
  // 所有者默认创建者；创建者转移所有权后仅为普通成员（审计仍记录创建者）
  task.ownerIdentityId = idOrNull(task.ownerIdentityId) || task.creatorIdentityId || null;
...
export function createTask(input, tasks = [], actor = "我", workflow) {
  const normalized = normalizeTask(input, workflow);
  ...
  const effectiveStatus = workflow?.mode !== "custom" && !STATUSES.includes(input.status) && normalized.assigneeIdentityIds.length ? "todo" : normalized.status;
  const order = Math.max(-1, ...tasks.filter((item) => item.status === effectiveStatus).map((item) => item.order ?? 0)) + 1;
  const task = {
    id: input.id || crypto.randomUUID(), title: normalized.title, description: normalized.description, descriptionText: normalized.descriptionText,
    status: effectiveStatus, ...(workflow ? { statusDefinition: resolveStatus(effectiveStatus, workflow), statusValue: resolveStatus(effectiveStatus, workflow)?.value, statusRevision: workflow.revision } : {}), priority: normalized.priority, tags: normalized.tags,
    creator: text(input.creator || actor, 50, "我") || "我", creatorIdentityId: idOrNull(input.creatorIdentityId),
    assigneeIdentityId: normalized.assigneeIdentityIds[0] || null, assigneeIdentityIds: normalized.assigneeIdentityIds, memberGrants: normalized.memberGrants, parentTaskId: normalized.parentTaskId,
    participantIdentityIds: normalized.participantIdentityIds,
    ...
    comments: [], history: [], watchers: [idOrNull(input.creatorIdentityId), ...normalized.assigneeIdentityIds].filter(Boolean)
  ...
  if (workflow?.mode === "custom") {
    const definition = task.statusDefinition;
    task.startedAt = definition.lifecycle === "active" ? createdAt : null;
    task.endedAt = definition.lifecycle === "terminal" ? createdAt : null;
    task.completedAt = definition.outcome === "completed" ? createdAt : null;
    task.cancelledAt = definition.outcome === "abandoned" ? createdAt : null;
  }
