...
export async function chatCompletion({
  ...
  jsonMode = false, stream = false, onDelta, signal, thinking
}) {
  ...
  // 百炼 DeepSeek V4 默认思考；报告整理可关闭。仅向已确认支持的服务/模型传扩展参数。
  // https://help.aliyun.com/zh/model-studio/deepseek-api
  if (thinking === false) {
    const host = new URL(url).hostname;
    const dashscope = /^dashscope(?:-intl|-us)?\.aliyuncs\.com$/.test(host) || host.endsWith(".maas.aliyuncs.com");
    if (dashscope && ["deepseek-v4-pro", "deepseek-v4-pro-0813", "deepseek-v4-flash", "deepseek-v4-flash-0731"].includes(model)) {
