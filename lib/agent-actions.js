import { resolveStatus, defaultWorkflow } from "../shared/task-statuses.js";
...
import {
  applyStatusTransition, createProgressRecord,
  normalizeTransitionReason, validateStatusTransition
} from "./tasks.js";
...
export function createAgentActionDraft(raw, allTasks, context, intent, sourceText = intent) {
  ...
  const actions = inputs.map((input) => {
    ...
    seen.add(task.id);
    const targetStatus = text(input?.targetStatus, 80) || null;
    ...
    if (targetStatus) {
      ...
      if (!resolveStatus(targetStatus, context.statusWorkflow)) throw actionError("AGENT_STATUS_INVALID", "目标状态不合法");
      ...
      validateStatusTransition(task.status, targetStatus, context.statusWorkflow);
    }
    ...
    return {
      ...
      targetStatus: targetStatus ? resolveStatus(targetStatus, context.statusWorkflow).id : null,
      reason,
      progressText,
  ...
  return {
    ...
    workflowRevision: context.statusWorkflow?.revision || 0, atomic: true, createdAt: new Date().toISOString(), sourceText: String(sourceText || "").slice(0, 2000), actions
  };
}
...
export async function confirmAgentActionDraft(ctx, context, draft) {
  try {
    ...
    if ((draft.workflowRevision || 0) !== workflow.revision) throw actionError("AGENT_PLAN_STALE", "状态流程已变化，请重新生成草稿", 409);
    ...
    for (const action of validated.actions) {
      ...
      if (action.targetStatus) {
        applyStatusTransition(task, action.targetStatus, {
          workflow: context.statusWorkflow, prevStatus: action.currentStatus, actor: context.actor.displayName, reason: action.reason
        });
      }
