...
describe("TaskDetailModal team assignment", () => {
  it("分派父任务并在详情中展示成员初始状态", async () => {
    ...
    const fetchMock = vi.fn((path, options = {}) => {
      ...
      if (path === "/api/tasks/parent-1/assign" && options.method === "POST") return response({ parent: { ...task, assignees: ["成员甲"], participants: [{ identityId: "member-a", displayName: "成员甲", status: "todo", executionTaskId: "execution-1" }] }, executions: [{ id: "execution-1" }], createdCount: 1 }, 201);
