...
export default function TeamMembersDrawer({ onClose = () => {}, returnFocusRef, inline = false, hideMembersSection = false, hideTimezoneSection = false, hidePendingSection = false }) {
    ...
    <aside className={isInline ? "team-members-inline-panel" : "team-drawer team-members-drawer"} role={isInline ? undefined : "dialog"} aria-modal={isInline ? undefined : true} aria-label="工作区成员管理" ref={dialogRef}>
      ...
      <div className="team-drawer-body team-members-drawer-body">
        ...
        {state.status === "ready" && <>
          ...
          {canManageScopes && !hidePendingSection && state.invitations.length > 0 && <section className="team-management-section"><header><h3>待对方确认</h3><span>{state.invitations.length} 人</span></header><div className="team-pending-invitations">{state.invitations.map((invitation) => <article key={invitation.id}><div><strong>{invitation.invitee.displayName}</strong><span>{invitation.invitee.email}</span></div><button type="button" disabled={Boolean(busy)} onClick={() => revokeInvitation(invitation)}>撤回</button></article>)}</div></section>}
          ...
          <section className="team-management-section team-audit-section"><header><h3>最近操作</h3><span>最新 {recentEvents.length} 条</span></header><ol className="team-audit-list">{recentEvents.length ? recentEvents.map((event) => {
            ...
            return <li key={event.id}><span className={`team-audit-outcome is-${event.outcome}`} aria-hidden="true" /><div><strong>{event.actor?.displayName || "系统"}</strong><span>{formatAuditMessage(event)}</span>{refs.length > 0 && <small>{refs.join(" · ")}</small>}</div><time>{formatTime(event.occurredAt)}</time></li>;
