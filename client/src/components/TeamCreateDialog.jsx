...
const requestId = () => globalThis.crypto?.randomUUID?.() || `workspace-${Date.now()}-${Math.random().toString(16).slice(2)}`;
...
export default function TeamCreateDialog({ onClose, onCreated }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(submitError.message || "工作区创建失败");
      setSaving(false);
    }
  };
  ...
  return <div className="board-modal-mask workspace-create-mask" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
    <form className="board-detail-modal board-confirm-modal workspace-create-dialog" role="dialog" aria-modal="true" aria-label="创建工作区" onSubmit={submit}>
      <header className="board-detail-head"><h2>创建工作区</h2><RadialRevealButton type="button" className="shell-icon-button" variant="icon" aria-label="关闭创建工作区" disabled={saving} onClick={onClose}>×</RadialRevealButton></header>
