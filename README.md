...
> **当前版本：v2.6.1**。本版本把卡片评论与回复纳入报告进展素材，与进展记录一起归纳成一句真实过程，并修复报告占位文字与内部状态码问题；保留报告模板、丰富描述编辑器、优先级配色与任务所有权。部署时使用 v2.6.1 配套的 Compose 文件与离线镜像包。
...
维护者重新制作 **v2.6.1 发布源码**的离线包时，在对应源码根目录执行；新版本应同步替换镜像标签和发布信息，不用旧标签覆盖未发布功能：

```bash
docker buildx build --platform linux/amd64 --load -f docker/Dockerfile -t nmtaskboard:2.6.1 .
docker pull --platform linux/amd64 postgres:16-alpine
docker save --platform linux/amd64 -o docker/nmtaskboard-linux-amd64.tar nmtaskboard:2.6.1 postgres:16-alpine
```
