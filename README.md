...
第一次会下载依赖并自动在本机拉起内置 PostgreSQL，可能要一两分钟。之后再开就很快。数据保存在项目里的 `data/` 目录（不要删，除非你想清空看板）。关掉窗口即停止服务。Windows 请使用 **64 位 Node.js**（`node -p process.arch` 应为 `x64`），项目路径尽量用英文。若提示端口占用，关掉另一个看板窗口，或打开 http://127.0.0.1:3301 。
