...
if (!databaseUrl) {
  ...
  test("管理员事务分派为成员创建唯一、独立且可追溯的执行任务", async (t) => {
    ...
    for (const person of [{ id: "member-a", name: "成员甲" }, { id: "admin-b", name: "管理员乙" }, { id: "outsider", name: "外部成员" }]) {
    ...
    await pool.end();
    for (const id of ["member-a", "admin-b"]) await requestJson(`${baseUrl}/api/team/members/invite`, { method: "POST", headers: { cookie: ownerCookie, "content-type": "application/json" }, body: JSON.stringify({ identifier: id }) });
    ...
    await verify.end();
    assert.deepEqual(counts.rows[0], { executions: 1, progress: 1 });
